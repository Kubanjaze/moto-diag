...
| candidates | `candidates.py`, no model | library files that name the spelling, cut to ±40 lines with path and page; none → `no_evidence` **without a model call**. A manual a manual route fetched for the spelling (sidecar `for_spellings`) is cut around its mechanism lines anywhere in it, never a contents line (`anchor: "route"`) | an agent-loop source stage browsed its way to 7.66M tokens for one spelling; the Vino 125 manual prints only "YJ125Y" |
| identity | `entry_check.list_identity`, no model | a manual-route PDF's model line is its hash-pinned list record's `dispModelName` ("VINO 125 - YJ125Y") beside its pdffileURL, **exact** name only; it serves `names_model` and E4, and the code becomes an alias | a model-code-only manual failed names_model one step after the excerpts |
| dry run | `entry_check.mechanism_lines`, no model | a spelling is sent only if an excerpt sentence passes E12 or names V-matic, CVT, DCT, Y-AMT, AMT, centrifugal, direct drive, V-belt, variator or single-speed, or "automatic" within three words of transmission, gearbox, belt, clutch or drive (a miss drops a spelling silently; a false alarm costs one call); the rest are `no_evidence`, "no mechanism line in the fetched pages"; counts in `summary.mechanism_lines`. Table-of-contents lines (a dot leader or a trailing page reference) are dropped first | Triumph and Yamaha spec pages give the gearbox only as table cells: 24 calls, nothing writable; Yamaha's MT-09 was sent on "6-16 Clutch lever ......" |
