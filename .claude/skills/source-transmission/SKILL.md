...
| dry run | `entry_check.mechanism_lines`, no model | a spelling is sent only if an excerpt sentence passes E12 or names V-matic, CVT, DCT, Y-AMT, AMT, centrifugal or direct drive; the rest are `no_evidence`, "no mechanism line in the fetched pages"; counts in `summary.mechanism_lines` | Triumph and Yamaha spec pages give the gearbox only as table cells: 24 calls, nothing writable |
