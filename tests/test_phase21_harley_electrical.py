"""Phase 21 — Harley electrical systems (all eras) knowledge base tests."""
...
def db_path(tmp_path):
    ...
    init_db(path)
    f = SEED_DATA_DIR / "knowledge" / "known_issues_harley_electrical.json"
    if f.exists():
        load_known_issues_file(f, path)
    return path


class TestHarleyElectricalData:
    def test_loads(self, db_path):
        assert count_known_issues(db_path=db_path) == 10

    def test_cross_era_coverage(self, db_path):
        results = search_known_issues(year=2000, db_path=db_path)
        assert len(results) >= 5

    def test_charging_dtc(self, db_path):
    ...
    def test_forum_tips(self, db_path):
        results = search_known_issues(query="regulator", db_path=db_path)
        assert len(results) >= 1
