"""Phase 77 — Cross-platform drivetrain diagnostics knowledge base tests."""
...
def db_path(tmp_path):
    ...
    init_db(path)
    f = SEED_DATA_DIR / "knowledge" / "known_issues_cross_platform_drivetrain.json"
    if f.exists():
        load_known_issues_file(f, path)
    return path


class TestCrossPlatformDrivetrainData:
    ...
    def test_high_severity(self, db_path):
        high = search_known_issues(severity="high", db_path=db_path)
        critical = search_known_issues(severity="critical", db_path=db_path)
        assert len(high) + len(critical) >= 3

    def test_noise(self, db_path):
        results = find_issues_by_symptom("noise", db_path)
        assert len(results) >= 4

    def test_vibration(self, db_path):
        results = find_issues_by_symptom("vibration at speed", db_path)
        assert len(results) >= 2

    def test_forum_tips(self, db_path):
        results = search_known_issues(query="chain", db_path=db_path)
