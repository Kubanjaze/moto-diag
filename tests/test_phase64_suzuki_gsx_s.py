"""Phase 64 — Suzuki GSX-S750/1000 + Katana knowledge base tests."""
...
def db_path(tmp_path):
    ...
    init_db(path)
    f = SEED_DATA_DIR / "knowledge" / "known_issues_suzuki_gsx_s.json"
    if f.exists():
        load_known_issues_file(f, path)
    return path


class TestSuzukiGsxSData:
    def test_loads(self, db_path):
        assert count_known_issues(db_path=db_path) == 10

    def test_year_range(self, db_path):
        results = search_known_issues(year=2022, make="Suzuki", db_path=db_path)
