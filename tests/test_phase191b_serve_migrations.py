...
class TestServeAppliesMigrationsByDefault:
    ...
    def test_serve_applies_pending_migration_to_match_schema_version(
        ...
        # Phase 255 / F124: the literal head-pin that stood here was deleted.
        # It duplicated SCHEMA_VERSION and caught nothing that
        # tests/test_phase240c_severity_ordering.py::test_schema_version_matches_the_migration
        # does not already catch by comparing the constant to max(MIGRATIONS).
        # Schema history lives in each Migration.description in
        # src/motodiag/core/migrations.py, not in a test comment.
