"""Phase 54 — Kawasaki dual-sport KLR650/KLX/Versys knowledge base tests."""
...
def db_path(tmp_path):
    ...
    init_db(path)
    f = SEED_DATA_DIR / "knowledge" / "known_issues_kawasaki_dual_sport.json"
    if f.exists():
        load_known_issues_file(f, path)
    return path


class TestKawasakiDualSportData:
    def test_loads(self, db_path):
        assert count_known_issues(db_path=db_path) == 10

    def test_year_range(self, db_path):
        results = search_known_issues(year=2015, make="Kawasaki", db_path=db_path)
    ...
    def test_forum_tips(self, db_path):
        results = search_known_issues(query="doohickey", db_path=db_path)
        assert len(results) >= 1
