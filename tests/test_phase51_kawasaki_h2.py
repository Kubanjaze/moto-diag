"""Phase 51 — Kawasaki Ninja H2 / H2R (2015+) knowledge base tests."""
...
def db_path(tmp_path):
    ...
    init_db(path)
    f = SEED_DATA_DIR / "knowledge" / "known_issues_kawasaki_h2.json"
    if f.exists():
        load_known_issues_file(f, path)
    return path


class TestKawasakiH2Data:
    ...
    def test_forum_tips(self, db_path):
        results = search_known_issues(query="supercharger", db_path=db_path)
        assert len(results) >= 1
