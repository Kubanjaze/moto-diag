"""Phase 246, part 2 — the five generic BMS entries, anchored per make.

Every entry in `known_issues_bms.json` is one of five concepts — cell
balancing, state of health, the voltage curve, thermal derating, cycle
counting — written as *what the BMS does* and *how each make shows it*, and
anchored to a manufacturer document for at least one make, or not written.
The corpus has no citation column; 244's convention is to name the document
inside the description, and these tests hold 246 to it.

The operator's rule for numbers: a numeric threshold is either
`service-manual` with the document named, or `forum` — never
`model-generated`, never `unverified`. The label's display is Part 1's job;
this half proves every number carries one.
"""
...
#: One row per concept, named in the *title* — Scope 2: `kb search` returns one
#: answer per concept. Word-bounded: a first draft matched "derat" anywhere and
#: "moderate" in the community cell-spread row kept the thermal check green
#: after the thermal row was deleted (mutation 6).
CONCEPTS = {
    "cell balancing": r"\bcell balancing\b",
    "state of health": r"\bstate of health\b",
    "voltage curve": r"\bvoltage(?:-to-state-of-charge)? curve\b",
...
def _entries() -> list[dict]:
    d = json.loads(BMS.read_text(encoding="utf-8"))
...
@pytest.fixture
def db(tmp_path):
    path = str(tmp_path / "phase246.db")
    init_db(path)
    load_known_issues_file(BMS, path)
    return path
...
class TestTheFiveConcepts:
    def test_the_seed_file_exists_and_loads(self, db):
        assert search_known_issues(db_path=db), "known_issues_bms.json loaded nothing"
...
class TestEveryEntryIsAnchored:
    @pytest.mark.parametrize("entry", _entries() if BMS.exists() else [], ids=_ident)
    ...
    def test_a_number_is_never_unlabelled(self, entry):
        ...
        if _NUMBER.search(_text(entry)):
            assert entry["source"] in {"service-manual", "forum"}, (
    ...
    def test_a_forum_entry_names_where_it_came_from(self, entry):
        ...
        owners"). The Unofficial Zero Manual was never fetched (expired TLS),
        so nothing may cite it. A forum entry names its site and, because the
        site revises its own thresholds, the page's "Last updated" date."""
    ...
    def test_nothing_is_model_generated(self):
        """The class 244Z's lesson says to minimise. Zero here: an entry that
        could not be anchored was not written."""
...
class TestReachable:
    ...
    @pytest.mark.parametrize("term", ["balancing", "state of health", "derating"])
