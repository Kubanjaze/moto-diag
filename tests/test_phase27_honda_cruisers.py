"""Phase 27 — Honda cruisers Shadow/VTX knowledge base tests."""
...
def db_path(tmp_path):
    ...
    init_db(path)
    f = SEED_DATA_DIR / "knowledge" / "known_issues_honda_cruisers.json"
    if f.exists():
        load_known_issues_file(f, path)
    return path


class TestHondaCruiserData:
    def test_loads(self, db_path):
        assert count_known_issues(db_path=db_path) == 10

    def test_year_range(self, db_path):
        results = search_known_issues(year=2005, make="Honda", db_path=db_path)
    ...
    def test_oil_leak_symptom(self, db_path):
        results = find_issues_by_symptom("oil leak", db_path)
        assert len(results) >= 1

    def test_forum_tips(self, db_path):
        results = search_known_issues(query="shaft drive", db_path=db_path)
        assert len(results) >= 1
