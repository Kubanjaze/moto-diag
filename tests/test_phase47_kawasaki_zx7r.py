"""Phase 47 — Kawasaki ZX-7R / ZX-7RR (1996-2003) knowledge base tests."""
...
def db_path(tmp_path):
    ...
    init_db(path)
    f = SEED_DATA_DIR / "knowledge" / "known_issues_kawasaki_zx7r.json"
    if f.exists():
        load_known_issues_file(f, path)
    return path


class TestKawasakiZX7RData:
    def test_loads(self, db_path):
        assert count_known_issues(db_path=db_path) == 10

    def test_year_range(self, db_path):
        results = search_known_issues(year=2000, make="Kawasaki", db_path=db_path)
    ...
    def test_forum_tips(self, db_path):
        results = search_known_issues(query="diaphragm", db_path=db_path)
        assert len(results) >= 1
