"""Phase 59 — Suzuki GSX-R1000 knowledge base tests."""
...
def db_path(tmp_path):
    ...
    init_db(path)
    f = SEED_DATA_DIR / "knowledge" / "known_issues_suzuki_gsx_r1000.json"
    if f.exists():
        load_known_issues_file(f, path)
    return path


class TestSuzukiGsxR1000Data:
    ...
    def test_forum_tips(self, db_path):
        results = search_known_issues(query="engine cases", db_path=db_path)
        assert len(results) >= 1
