...
class TestRegression:
    ...
    def test_schema_version_pin(self):
        assert SCHEMA_VERSION == 54, (  # f9-noqa: ssot-pin contract-pin: Gate 11 schema-bump pin. The literal is the point — importing the constant would make this assert `x == x` and it would never fail. Bumped 51→52 at Phase 235B (migration 052 rebuilt known_issues to widen the source CHECK with `regulation`, for entries quoted from primary legal text). Previously 50→51 at Phase 211 (migration 051 added known_issues.source). Bumping requires a corresponding new migration in src/motodiag/core/migrations.py. Bumped 52→53 at Phase 240C (migration 053 replaces idx_known_issues_sort with an expression index on the severity rank, because the ordering fix moved the queries from `ORDER BY severity DESC` — lexicographic, `critical` last — to a CASE rank that the old index cannot serve). Bumped 53→54 at Phase 244D (migration 054 deduplicates known_issues — 6,600 rows for 660 distinct issues, every entry present exactly ten times, because the table had no uniqueness constraint and the loader was not idempotent — and adds a UNIQUE EXPRESSION index over (COALESCE(make,''), COALESCE(model,''), title); COALESCE rather than a plain column constraint because 43 seeded entries carry a NULL model and SQLite treats NULLs as DISTINCT in a UNIQUE constraint, so the naive form would have left those free to keep duplicating behind something that looked like a fix).
