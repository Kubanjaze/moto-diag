"""Phase 50 — Kawasaki ZX-12R / ZX-14R (2000-2020) knowledge base tests."""
...
def db_path(tmp_path):
    ...
    init_db(path)
    f = SEED_DATA_DIR / "knowledge" / "known_issues_kawasaki_hyperbike.json"
    if f.exists():
        load_known_issues_file(f, path)
    return path


class TestKawasakiHyperbikeData:
    ...
    def test_forum_tips(self, db_path):
        results = search_known_issues(query="ram air", db_path=db_path)
        assert len(results) >= 1
