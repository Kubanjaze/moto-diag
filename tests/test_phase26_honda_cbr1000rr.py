"""Phase 26 — Honda CBR1000RR / RR-R (2004+) knowledge base tests."""
...
def db_path(tmp_path):
    ...
    init_db(path)
    f = SEED_DATA_DIR / "knowledge" / "known_issues_honda_cbr1000rr.json"
    if f.exists():
        load_known_issues_file(f, path)
    return path


class TestHondaCBR1000RRData:
    ...
    def test_forum_tips(self, db_path):
        results = search_known_issues(query="HSTC", db_path=db_path)
        assert len(results) >= 1
