"""Phase 71 — Cross-platform fuel injection troubleshooting knowledge base tests."""
...
def db_path(tmp_path):
    ...
    init_db(path)
    f = SEED_DATA_DIR / "knowledge" / "known_issues_cross_platform_fi.json"
    if f.exists():
        load_known_issues_file(f, path)
    return path


class TestCrossPlatformFiData:
    def test_loads(self, db_path):
        assert count_known_issues(db_path=db_path) == 10

    def test_multi_make(self, db_path):
        # Issues should reference multiple makes
