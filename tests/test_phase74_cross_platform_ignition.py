"""Phase 74 — Cross-platform ignition system diagnostics knowledge base tests."""
...
def db_path(tmp_path):
    ...
    init_db(path)
    f = SEED_DATA_DIR / "knowledge" / "known_issues_cross_platform_ignition.json"
    if f.exists():
        load_known_issues_file(f, path)
    return path


class TestCrossPlatformIgnitionData:
    def test_loads(self, db_path):
        assert count_known_issues(db_path=db_path) == 10

    def test_multi_make(self, db_path):
        """Cross-platform ignition phases should reference multiple makes."""
    ...
    def test_forum_tips(self, db_path):
        results = search_known_issues(query="spark plug", db_path=db_path)
        assert len(results) >= 1
        found_forum_tip = any(
            "Forum tip" in r.get("fix_procedure", "") for r in results
        )
        assert found_forum_tip, "Expected at least one spark plug result with 'Forum tip' in fix_procedure"
