"""Phase 32 — Honda vintage air-cooled CB550/650/750, Nighthawk knowledge base tests."""
...
def db_path(tmp_path):
    ...
    init_db(path)
    f = SEED_DATA_DIR / "knowledge" / "known_issues_honda_vintage.json"
    if f.exists():
        load_known_issues_file(f, path)
    return path


class TestHondaVintageData:
    def test_loads(self, db_path):
        assert count_known_issues(db_path=db_path) == 10

    def test_year_range(self, db_path):
        results = search_known_issues(year=1975, make="Honda", db_path=db_path)
    ...
    def test_oil_leak(self, db_path):
