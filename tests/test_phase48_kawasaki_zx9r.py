"""Phase 48 — Kawasaki ZX-9R (1998-2003) knowledge base tests."""
...
def db_path(tmp_path):
    ...
    init_db(path)
    f = SEED_DATA_DIR / "knowledge" / "known_issues_kawasaki_zx9r.json"
    if f.exists():
        load_known_issues_file(f, path)
    return path


class TestKawasakiZX9RData:
    def test_loads(self, db_path):
        assert count_known_issues(db_path=db_path) == 10

    def test_year_range(self, db_path):
        results = search_known_issues(year=2001, make="Kawasaki", db_path=db_path)
