"""Phase 68 — Suzuki electrical systems + FI dealer mode knowledge base tests."""
...
def db_path(tmp_path):
    ...
    init_db(path)
    f = SEED_DATA_DIR / "knowledge" / "known_issues_suzuki_electrical.json"
    if f.exists():
        load_known_issues_file(f, path)
    return path


class TestSuzukiElectricalData:
    ...
    def test_forum_tips(self, db_path):
        results = search_known_issues(query="C-mode", db_path=db_path)
