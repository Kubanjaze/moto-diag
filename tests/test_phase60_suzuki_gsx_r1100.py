"""Phase 60 — Suzuki GSX-R1100 (1986-1998) knowledge base tests."""
...
def db_path(tmp_path):
    ...
    init_db(path)
    f = SEED_DATA_DIR / "knowledge" / "known_issues_suzuki_gsx_r1100.json"
    if f.exists():
        load_known_issues_file(f, path)
    return path


class TestSuzukiGsxR1100Data:
    def test_loads(self, db_path):
        assert count_known_issues(db_path=db_path) == 10

    def test_year_range(self, db_path):
        results = search_known_issues(year=1993, make="Suzuki", db_path=db_path)
    ...
    def test_forum_tips(self, db_path):
        results = search_known_issues(query="oil cooler", db_path=db_path)
        assert len(results) >= 1
