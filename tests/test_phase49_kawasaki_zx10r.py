"""Phase 49 — Kawasaki ZX-10R (2004+) knowledge base tests."""
...
def db_path(tmp_path):
    ...
    init_db(path)
    f = SEED_DATA_DIR / "knowledge" / "known_issues_kawasaki_zx10r.json"
    if f.exists():
        load_known_issues_file(f, path)
    return path


class TestKawasakiZX10RData:
    ...
    def test_forum_tips(self, db_path):
        results = search_known_issues(query="KTRC", db_path=db_path)
        assert len(results) >= 1
