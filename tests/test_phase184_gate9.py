...
class TestGate9AntiRegression:
    ...
    def test_schema_version_unchanged(self):
        # Gate 9 anti-regression pin. Last migration is 052 (Phase
        # 235B: known_issues.source gains `regulation`, for content
        # quoted from primary legal text). Bump this pin alongside any
        ...
        assert SCHEMA_VERSION == 63  # f9-noqa: ssot-pin contract-pin: Gate 9 schema-bump pin (Phase 184 anti-regression). Bumped 51→52 at Phase 235B (migration 052 rebuilt known_issues to widen the source CHECK with `regulation`). Bumping requires a corresponding new migration in src/motodiag/core/migrations.py + verifying the schema diff at the next migration sequence number; this test is the primary loud-fail surface for accidental SCHEMA_VERSION bumps shipping without their migration. Bumped 52→53 at Phase 240C (migration 053 replaces idx_known_issues_sort with an expression index on the severity rank, because the ordering fix moved the queries from `ORDER BY severity DESC` — lexicographic, `critical` last — to a CASE rank that the old index cannot serve). Bumped 53→54 at Phase 244D (migration 054 deduplicates known_issues — 6,600 rows for 660 distinct issues, every entry present exactly ten times, because the table had no uniqueness constraint and the loader was not idempotent — and adds a UNIQUE EXPRESSION index over (COALESCE(make,''), COALESCE(model,''), title); COALESCE rather than a plain column constraint because 43 seeded entries carry a NULL model and SQLite treats NULLs as DISTINCT in a UNIQUE constraint, so the naive form would have left those free to keep duplicating behind something that looked like a fix). Bumped 54→55 at Phase 244F (migration 055 adds the known_issue_makes junction: `known_issues.make` is one free-text column holding a marque, a list of marques, a scope phrase and in one row a whole sentence of findings, so LiveWire and Damon were not queryable makes AT ALL — all 24 LiveWire rows sit inside 'Harley-Davidson, LiveWire' and Phase 243's entire output was unreachable. The column is not modified; the junction is derived from it by knowledge/marques.extract_marques). Bumped 55→56 at Phase 244I (migration 056 adds the known_issue_models junction: the model column has the same list-and-prose disease as make, plus entries that name models in order to EXCLUDE them, so extraction is clause-scoped). Bumped 56→57 at Phase 244L (migration 057 widens the cost_events kind CHECK to accept vision_sweep and vision_guidance: vision spent money from Phase 191B but the constraint rejected the rows, so the ledger sat empty). Bumped 57→58 at Phase 244M (migration 058 adds memory_facts, the per-machine compiled memory: keyed on vehicle_id and deliberately NOT on customer_id, because `customers` row 1 is the `Unassigned` sentinel that every vehicle row carries by DEFAULT, so a customer key would compile one memory holding every bike in the shop -- the exact cross-contamination the feature exists to prevent; fact_key is a UNIQUE hash that COALESCEs its nullable origin_id, because SQLite treats NULLs as DISTINCT and the naive form would let every re-compile duplicate). Bumped 58→59 at Phase 244N (migration 059 adds guidance_interactions and video_analyses: /ask discarded its whole answer while 244L recorded what the question cost, and set_analysis_findings overwrote the findings blob so every re-analysis destroyed the prior sweep). Bumped 59→60 at Phase 244Q (migration 060 widens the cost_events kind CHECK to accept text_diagnosis: the TEXT diagnosis path spent money the ledger could not hold, so the first half of 244Q tuned max_tokens against spend nobody could measure). Bumped 60→61 at Phase 209D (migration 061 adds diagnostic_sessions.shop_id: a session had no shop and no link to a work order, so every cost_events row for vision and text carried shop_id=NULL and a monthly cap read through shop_cost_this_month would have read $0 for every shop and never fired -- a safeguard that looks like it works). Bumped 61→62 at Phase 244R (migration 062 classifies the 99 seeded dtc_codes rows: knowledge/loader.py built every DTCCode without dtc_category, so the field took its default and `motodiag code --category engine` answered 'No DTCs found' over 29 engine codes — 19 of the 20 categories behaved that way, and the twentieth, 'unknown', returned the whole table. The migration also rewrites the emissions and exhaust descriptions, which both claimed O2 and catalyst, so the authority could not settle where P0420 belongs). Bumped 62→63 at Phase 255 (migration 063 adds the transmission axis: `vehicles.transmission`, a per-axis typed column with a CHECK constraint and — unlike `powertrain`, which defaults to 'ice' — NULLABLE with no default, because a machine whose transmission nobody recorded has an unknown transmission and writing 'manual' would be a fabrication retrieval then acts on; and `known_issues.applicability`, a JSON object keyed by axis. Phase 254 shipped twelve rows about scooter CVTs whose make column names seven marques, so a Gold Wing retrieved 9 of them, a CBR1000RR 9, a Grom 9 and an XS650 10, while a Kawasaki Ninja 400 retrieved 0 — the make column, not the content. The post_apply hook backfills those rows because a loader fix only reaches a database someone re-seeds). NOTE: this bump had to touch SEVEN exact-head pins, not the three the first blast radius found — 184, 240, 244D, 244I, 244M, 244N and 191b — because they are spread across files a subsystem-scoped blast radius does not include. Only the full regression finds them all. Reminder: test_phase191b_serve_migrations.py spells its pin `get_current_version(db_path) == N`, so grepping only for SCHEMA_VERSION misses it.
