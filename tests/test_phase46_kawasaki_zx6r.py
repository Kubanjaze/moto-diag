"""Phase 46 — Kawasaki ZX-6R (1995+) knowledge base tests."""
...
def db_path(tmp_path):
    ...
    init_db(path)
    f = SEED_DATA_DIR / "knowledge" / "known_issues_kawasaki_zx6r.json"
    if f.exists():
        load_known_issues_file(f, path)
    return path


class TestKawasakiZX6RData:
    ...
    def test_forum_tips(self, db_path):
        results = search_known_issues(query="KLEEN", db_path=db_path)
        assert len(results) >= 1
