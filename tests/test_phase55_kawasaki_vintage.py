"""Phase 55 — Kawasaki vintage KZ/GPz knowledge base tests."""
...
def db_path(tmp_path):
    ...
    init_db(path)
    f = SEED_DATA_DIR / "knowledge" / "known_issues_kawasaki_vintage.json"
    if f.exists():
        load_known_issues_file(f, path)
    return path


class TestKawasakiVintageData:
    def test_loads(self, db_path):
        assert count_known_issues(db_path=db_path) == 10

    def test_year_range(self, db_path):
        results = search_known_issues(year=1982, make="Kawasaki", db_path=db_path)
