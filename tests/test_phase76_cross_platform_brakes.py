"""Phase 76 — Cross-platform brake system diagnostics knowledge base tests."""
...
def db_path(tmp_path):
    ...
    init_db(path)
    f = SEED_DATA_DIR / "knowledge" / "known_issues_cross_platform_brakes.json"
    if f.exists():
        load_known_issues_file(f, path)
    return path


class TestCrossPlatformBrakesData:
    ...
    def test_forum_tips(self, db_path):
        results = search_known_issues(query="brake fluid", db_path=db_path)
        assert len(results) >= 1
