"""Phase 29 — Honda standards CB750/919, CB1000R, Hornet knowledge base tests."""
...
def db_path(tmp_path):
    ...
    init_db(path)
    f = SEED_DATA_DIR / "knowledge" / "known_issues_honda_standards.json"
    if f.exists():
        load_known_issues_file(f, path)
    return path


class TestHondaStandardsData:
    def test_loads(self, db_path):
        assert count_known_issues(db_path=db_path) == 10

    def test_year_range(self, db_path):
        results = search_known_issues(year=2015, make="Honda", db_path=db_path)
    ...
    def test_forum_tips(self, db_path):
        results = search_known_issues(query="handlebar", db_path=db_path)
        assert len(results) >= 1
