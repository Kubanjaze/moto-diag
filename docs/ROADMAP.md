...
| 191B | Video diagnostic upload + Claude Vision AI analysis pipeline | 🔲 | **NEW row at Phase 191 finalize** (per Kerwyn pre-plan ask: scope split must be visible on the ROADMAP, not buried in FOLLOWUPS). Backend feature continuation that consumes Phase 191's mobile capture substrate. New `/v1/videos/*` endpoints (POST upload multipart with .mp4 + JSON sidecar / GET list per session / DELETE / GET single + analysis findings). Migration adding `videos` table (id / session_id / file_path / sha256 / duration_ms / width / height / file_size_bytes / interrupted / upload_state / analysis_state / created_at / analyzed_at). ffmpeg integration for frame + audio extraction (key-frame sampling at configurable interval; audio waveform-relevant region detection). Claude Vision API wired through the new HTTP layer for the first time on Track I — extracts findings about visible bike issues (smoke colors, leak locations, exhaust pulse irregularities, throttle response) + audio-derived findings (idle stability, knock detection). Phase 100-103 Python modules (Claude API streaming + tool use + vision + structured outputs) wired into HTTP layer — first production usage outside the learning series. Mobile side: `useSessionVideos` hook swap from FS-backed to backend-backed; consumer surface (SessionDetailScreen's VideosCard, VideoCaptureScreen save flow) UNCHANGED per Phase 191's handoff contract — same `{videos, addRecording, deleteVideo, refresh, atCap, capReason}` shape, swap is invisible to call sites. The 4 backend-side fields stubbed null in Phase 191 (`remoteUrl`, `uploadState`, `analysisState`, future analysis findings) start populating. **Spawned from Phase 191's capture-only scope split.** |
