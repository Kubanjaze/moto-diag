...
| 225 | KTM electrical + Keihin FI | 🔲 | Keihin ECU, Tuneboy, PowerParts cross-platform. **Inherits a real adapter-catalog gap from Phase 221**: KTM has 4 compat rows across 3 adapters, every one `partial` or `read-only`, and **no full-access option** — compare BMW's 17 rows with the GS-911 and Ducati's 8 with the OEM DDS. 221 declined to fill it because this row owns KTM tooling, and guarded the row count at 4 so the decision is on the record. **Also check the row's own naming**: TuneBoy and TuneECU are different products, and row 230 names TuneECU for Triumph while this row names Tuneboy for KTM — verify which one actually covers KTM before writing. |
