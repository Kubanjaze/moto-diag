...
| 192B | Diagnostic report PDF export + Share Sheet (feature) | 🔲 | **Feature half of substrate-then-feature pair** (192 viewer / 192B PDF + Share Sheet). Mobile-side PDF download + iOS UIActivityViewController + Android ACTION_SEND wiring via `react-native-share`. Backend gains sibling `POST /v1/reports/session/{id}/pdf` for preset-filtered PDFs (composer-side filter; renderer stays pure per Phase 192 shape doc). GET sibling unchanged. Deterministic-rendering pytest at Commit 1 (fail-fast if reportlab non-deterministic; F34 candidate). Belt-and-suspenders temp-file cleanup (per-share unlink + 24hr-old startup sweep). 3 commits planned (1 backend + 2 mobile + 1 finalize). Plan v1.0 written 2026-05-05 with F33 process refinement applied (existing-code overlap audit BEFORE plan, caught Phase 192's PDF route + reportlab substrate already in place). |
