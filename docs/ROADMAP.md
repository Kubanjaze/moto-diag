...
| 194 | Camera + photo integration | 🚧 | **Substrate half of substrate-then-feature pair** (194 capture/attach/display + 194B AI photo analysis). Branch `phase-194-camera-photo-integration` BOTH repos. **CLAUDE.md Step 0 audit ran first** (dual-direction, second canonical-process invocation): backend has video-route pattern (Phase 191B) + KB photo_annotation surfaces (Phase 119, NOT applicable to per-WO uploads); mobile has `react-native-vision-camera@4.7.3` already installed (Phase 191) + `useSessionVideos`/`videoStorageCache`/`VideoCaptureScreen` direct templates. **NO new dependency on either side.** **Scope**: `work_order_photos` table (work_order_id NOT NULL + issue_id nullable + role enum {before/after/general/undecided} + pair_id self-FK + analysis_state/findings nullable for 194B-anticipating substrate) · `POST /v1/shop/{shop_id}/work-orders/{wo_id}/photos` multipart route + per-WO 30 / per-issue 10 quotas · backend image pipeline (HEIC→JPEG decode + EXIF orientation upright + resize 2048px long-edge + JPEG quality 85, single canonical storage format) · mobile `useWorkOrderPhotos` hook + `PhotoCaptureScreen` (4-state machine: idle/previewing/uploading/uploaded/failed) + `photoStorageCache` (DocumentDirectoryPath + 7-day cold-start orphan sweep) + `WorkOrderPhotosSection` discriminated-union variant (FIRST variant addition to Phase 193's `WorkOrderSection` substrate — load-bearing test of forward-look commitment) + 4-button capture-time fast-path (Before/After/General/Decide later) + post-capture re-classification surface. **3 commits planned** (backend Commit 0 + mobile Commits 1, 2). **F-tickets**: F37 (value-level enum mismatch) watching during execution; instance #3 escalates to file. F38 (ShopAccessError quota_exceeded distinctness) candidate. Plan v1.0 written 2026-05-06. |
