...
| 189 | Diagnostic session UI (mobile) | 🔲 | Sessions tab + list + new session + detail (symptoms/DTCs/diagnosis/close-reopen) over Phase 178 `/v1/sessions`. Tab nav introduced. DTC tap-to-lookup deferred to Phase 190. (was 178; swapped with 190 at plan time so the canonical mechanic workflow lands first) |
| 190 | DTC code lookup screen | 🔲 | Search by code or text, voice input, offline DTC database. Adds tap-from-SessionDetail integration into Phase 189's DTC list. (was 177; swapped with 189) |
