...
## Track I — Mobile App (iOS + Android) (Phases 185–204, +191B inserted at Phase 191 finalize → 21 phases)
...
| 191 | Video diagnostic capture (mobile, capture-only substrate) | ✅ | **Mobile-only capture substrate.** Video recording + on-device storage + playback inside `SessionDetailScreen`. Backend upload + Claude Vision AI analysis split off as Phase 191B (NEW row below) per substrate-then-feature pattern. Branch `phase-191-video-diagnostic-capture`, **8 commits** rebase-merged to mobile `main` (6 build + 1 in-cycle Commit-3 fix + 1 full-gate fix). 3 new runtime deps (vision-camera 4.7.3 + rn-fs 2.20.0 + rn-video 6.19.2) — zero patch-package work needed (clean build against `newArchEnabled=false` first try). 2 new screens (VideoCaptureScreen + VideoPlaybackScreen on SessionsStack), 2 new hooks (useCameraPermissions + useSessionVideos backend-agnostic Phase 191B handoff contract), 3 pure-helper modules (videoCaptureMachine reducer with 5 states + Kerwyn fold #1 auto-keep-on-background + Kerwyn fold #2 RecordingError discriminated union; videoStorage RNFS-backed FS policy with 5-video / 500MB / 100MB-free caps + EXDEV cross-volume save fallback; videoCaptureHelpers formatElapsed + formatFileSize + generateShortId + classifyVisionCameraError). VideosCard placed BETWEEN FAULT CODES and DIAGNOSIS in SessionDetail (Kerwyn pre-plan placement). Closed-session lockdown HIDES Record button; symmetric gap for Phase 189 append inputs filed as F7. **Commit 3 architect-smoke FAILED** at "⏸ Paused" badge verification — closure-state-capture bug: onRecordingFinished closure captured `state=idle` at registration-time, never saw the AppState handler's transition. Fix: `interruptedRef` useRef pattern + folded F8 formatFileSize unit-switching. **Architect full gate FAILED with 3 bugs**: Bug 1 HIGH VideosCard refresh on focus (SessionDetail doesn't unmount on push; useSessionVideos's mount-time effect didn't re-fire); Bug 2 LOW closed-with-videos visual hole; Bug 3 MEDIUM bottom-tab wireframe icons (NOT a Phase 191 regression — `@react-navigation/elements` default `MissingIcon` `⏷` glyph since Phase 189 commit 2; full gate just surfaced it). 3-bug fix landed as single commit `39948c1` with per-bug callouts. **Re-smoke GREEN** on 8/8 verifications + full sanity sweep (Garage, Sessions, DTC P0171, 2:20 PM Paused-badge artifact preserved on Session #1 as 191B regression coverage). 301/301 mobile tests, tsc clean every commit, mobile 0.0.5→0.0.6, backend 0.13.6→0.13.7. **Key finding: substrate-then-feature splits keep gates gate-sized** + closure-state-capture is the 3rd instance of "snapshot/assumption doesn't match runtime" failure family (Phase 188 HVE / Phase 190 substring / Phase 191 closure) — F9 filed for the useRef pattern doc + generalized lint rule. (was 179) |
| 191B | Video diagnostic upload + Claude Vision AI analysis pipeline | ✅ | **Substrate-then-feature pair completion** (Phase 191 substrate → 191B feature). Backend `/v1/sessions/{id}/videos` (POST upload + GET list/single + DELETE soft-delete + GET file-stream) over migration v39's `videos` table; ffmpeg subprocess wrapper; Vision pipeline reusing Phase 101's prompt + finding types unchanged with tool-use structured output via `DiagnosticClient.ask_with_images()` extension. Mobile hook swap (load-bearing assertion: 10 of 12 useSessionVideos.test.ts it() titles preserved verbatim); uploading-state machine + 5-state analysis badge. **5 architect-gate rounds**: round 1 BLOCKED at Step 1B (motodiag serve never called init_db; Phase 175 latent), round 2 BLOCKED at Step 6 (mobile FormData missing file:// prefix; mock-vs-fetch), round 3 BLOCKED at Step 5 by tier enforcement working as designed (added `motodiag subscription set --user N --tier T` CLI), round 4 BLOCKED at Step 7 by hardcoded fabricated Vision model ID `claude-sonnet-4-5-20241022` (corrected to `claude-sonnet-4-6` + 14 hardcoded test refs scrubbed; F15 regression guard), round 5 PASS on 22/22 steps. Full Vision pipeline ran end-to-end against live Anthropic API at $0.0354/video, 5 frames, model_used='claude-sonnet-4-6'. **6 instances of the F9 failure family on Track I (3 in Phase 191B alone)**; pattern is robust enough to merit Phase 192's lead ticket (architectural-pattern doc + lint rule). 8 backend commits + 3 mobile commits. 151 backend Phase 191B tests pass with zero skips (sample_3sec.mp4 fixture + parser fix at Commit 7 closed the 5 previously-gated TestRealFFmpeg skips); 293 mobile Jest tests across 21 suites all green. Backend `pyproject.toml` 0.1.0 → 0.2.0 + `python-multipart>=0.0.20` to `[api]` extras; backend `implementation.md` 0.13.7 → 0.13.8; schema 38 → 39; mobile `package.json` 0.0.6 → 0.0.7; mobile `implementation.md` 0.0.8 → 0.0.9. Phase 192 polish: F10 (session_repo UTC), F12 (FormData URI-prefix spec test), F13 (mobile 402 vs 403 UI), F16 (runbook hygiene for API key handling — leaked 4 times during smoke), F18 (image_quality_note not rendered in mobile), Issue 1 (post-wipe-data API key recovery), doc fix (drop --tier shop from `motodiag apikey create`). |
| 191D | F9 SSOT-constants lint generalization (extends 191C narrow rule + TAG_CATALOG coverage check) | ✅ | **Track I tooling phase, second F9-family intervention.** Branch `phase-191D-f9-ssot-constants-lint` BOTH repos, **4 commits** (Commit 1 both / Commit 2 backend / Commit 3 mobile / Commit 4 both finalize). Carries forward F20 + F21 from Phase 191B fix-cycle-5; same family as Phase 191C subspecies (ii) but generalized beyond model IDs. **Backend lint script** `scripts/check_f9_patterns.py` extended with `--check-ssot-constants` (TOML-driven 14-entry registry) + `--check-tag-catalog-coverage` modes; deprecated `--check-model-ids` as stub-redirect with stderr deprecation banner. **Mobile ESLint plugin** gains `motodiag/no-hardcoded-ssot-constants-in-tests` rule (severity error from day one; JSON registry with explicit `role: contract` field encodes contract-vs-default distinction at schema level rather than backend's TOML inline-comment approach). **Pattern doc extended** in both repos: layered-history note + Instances #8 (SCHEMA_VERSION) + #9 (TAG_CATALOG forward-direction) + #10 (TAG_CATALOG reverse-direction auth-orphan, 378-day-latency narrative) + new `contract-pin` opt-out category + recognition pattern. **Trust-but-verify caught 7 deviations** all folded into source commits: noise-literal filter on dict-typed entries (311 → 142 false positives) + tightened has_import to drop reverse-direction substring match (142 → 82) + dropped DEFAULT_VISION_MODEL from registry (82 → 17 legitimate) + stub-redirect double-fire bug (replaced with no-op stub) + pre-existing `atCap` unused-var fix + missing legacy `f9-allow-model-ids` back-compat + PER_SESSION_COUNT_CAP not exported. **Production cleanup**: `src/motodiag/intake/vehicle_identifier.py` HAIKU/SONNET_MODEL_ID literals → MODEL_ALIASES references (F24 promotion criterion data point 1) + auth tag orphan removed (Phase 183 forward-looking placeholder, 378 days latent). **Mobile inline cleanup**: MAX_VIDEOS_PER_SESSION SSOT consolidation from 2 screens to `src/types/video.ts` (resolves F25 inline). **Final lint state**: backend `--all` clean; mobile 0 motodiag/* findings (1 informational deprecation banner expected). **Tests**: 34/34 backend (191C + 191D suites) + 293/293 mobile Jest. **F-tickets**: F20 + F21 closed; F22 + F23 + F24 + F26 (NEW) filed; F25 explicitly NOT filed. Backend `pyproject.toml` 0.3.0 → 0.3.1 (patch); backend `implementation.md` 0.13.9 → 0.13.10; mobile `package.json` 0.0.8 → 0.0.9; mobile `implementation.md` 0.0.10 → 0.0.11. Schema unchanged at v39. **Key finding**: the value of a lint rule is most visible on its first run against an existing codebase, when latent drift gets surfaced all at once. |
| 191C | F9 failure-family architectural intervention (pattern doc + lint rules) | ✅ | **Track I tooling phase inserted before Phase 192** per architect's Phase 191B PASS-handoff observation. Branch `phase-191C-f9-architectural-intervention` BOTH repos, **6 commits** (Commit 1 both × 1 + Commit 2 backend + Commit 3 mobile + Commit 4 mobile + Commit 5a both + Commit 5b both). Plan v1.0 + v1.0.1 (5 corrections including 6→7 instance count, subspecies (v) "self-validating-test-setup" rename, Wave 2 parallel-dispatch). **Pattern guide doc** in both repos (backend 634 lines / 6,126 words / 15 code samples; mobile 686 lines / 6,462 words / 17 code samples) covers all 7 F9 case studies × (bug + mock-vs-runtime gap + anti-example + fix + recognition heuristic + lint coverage + commit hash). **Backend lint script** `scripts/check_f9_patterns.py` (376+ lines, two checks: `--check-model-ids` + `--check-deploy-path-init-db`) + `tests/test_phase191c_f9_lint.py` (17 tests) + `.pre-commit-config.yaml` + `pyproject.toml` 0.2.0 → 0.3.0 + `pre-commit>=3.5` to `[dev]`. **Mobile ESLint plugin** `eslint-plugin-motodiag/` with 3 rules — `no-closure-state-capture-in-native-callback` (subspecies i), `no-hardcoded-model-ids-in-tests` (subspecies ii mobile twin), `no-loose-typed-async-mock-returns` (subspecies iii) — each with RuleTester unit tests. **Husky + lint-staged** wiring on mobile. **5a clean-baseline cleanup** (`719de3b` backend / `3b0e439` mobile): rule refinement (`MIN_OPTOUT_REASON_CHARS = 20` floor + `FILE_OPTOUT_SCAN_LINES` 30→100 + `malformedOptOut` finding) + 4 file-level opt-outs (SSOT-pin / meta-test category) + 8-file refactor pass — backend 50 → 0 findings; mobile 2 → 0 findings. **5b severity bump + finalize** (this commit): mobile ESLint warn → error for all 3 motodiag/* rules; 2 backend strict-xfail clean-baseline gate tests un-xfailed (now 17/17 PASS); pyproject 0.2.0 → 0.3.0; mobile package.json 0.0.7 → 0.0.8; backend impl.md 0.13.8 → 0.13.9; mobile impl.md 0.0.9 → 0.0.10; F9 closed in mobile FOLLOWUPS with 5b commit hash. **Three operational asks accepted before 5a started**: (1) mandatory `<reason>` length floor on opt-out syntax (20 chars); (2) per-line bucketing for 14 ambiguous findings BEFORE editing — discipline "does the test break if SSOT changes?"; all 14 bucketed as REFACTOR; (3) sanity-check predicted count drop BEFORE 8-file refactor pass — caught the FILE_OPTOUT_SCAN_LINES 30→100 bug. **Architect-side paired-review of subspecies (ii)** confirmed no drift between backend + mobile rules. **F9 retroactive coverage**: 5 of 7 instances catchable by lint (subspecies i mobile + ii both stacks + iii mobile + iv backend); 2 doc-only (subspecies v self-validating-test-setup + iii's date-boundary cousin). **Architect gate**: implicit / self-passing because the gate's pre-conditions ARE the 5b commit (no native-module integration, no feature surface, no separate gate round needed for a docs+tooling phase). Schema unchanged at v39. **Key finding: the discipline of "does the test break if the SSOT changes?" is the load-bearing principle**, transferable across all 5 subspecies. The lint rules catch the symptom; the discipline determines the FIX (opt-out vs refactor). |
| 192 | Diagnostic report viewer (substrate) | ✅ | **Substrate half of substrate-then-feature pair** (192 viewer / 192B PDF + Share Sheet). Branch `phase-192-diagnostic-report-viewer` BOTH repos. **4 commits**: backend Commit 1 (`5a12195`) bundled videos section variant 5 + migration 040 (`videos.analyzing_started_at TEXT` nullable) + atomic worker update (Contract B single-UPDATE) + composer + renderer extension + F9 int-typed heuristic refinement (eliminated 22 false positives at SCHEMA_VERSION=40) + 4 new test files; mobile Commits 2-4 added `useReport` hook + `ReportDocument`/`ReportSection` discriminated-union types + 5 type-guards + `ReportViewerScreen` + `SectionToggle` 3-chip preset selector + `ReportSectionCard` discriminated-union renderer + 3 pure-logic helper modules (presets / Contract A stuck-detection / formatters) + SessionsStack registration + SessionDetail "View report" cross-link button. **Plan amendments**: v1.0 → v1.0.1 reshape (Phase 182 already shipped the route surface; pivoted to extension); v1.0.2 rigor refinements; v1.0.3 schema scope-add (path-α: don't ship known schema-vs-doc drift); v1.0.4 heuristic refinement folded into Commit 1. **Architect-side substrate-decision artifacts** (first explicit instance in project): F29 ADR `auth-policy.md` (owner-only-with-404 + tier-bypass-on-read documented as emergent pattern) + `report-document-shape.md` (5 section variants + reportlab-not-WeasyPrint correction + customer/insurance preset note for 192B) + Builder brief. **F-tickets**: F28 + F29 filed in mobile FOLLOWUPS (both deferred to Phase 192B). **Tests**: 4395 backend + 363 mobile (293 → 363, +70 across the phase). Schema 39 → 40. Backend `pyproject.toml` 0.3.1 → 0.3.2; backend `implementation.md` 0.13.10 → 0.13.11; mobile `package.json` 0.0.9 → 0.1.2; mobile `implementation.md` 0.0.11 → 0.1.2. **Key finding: substrate-decision-then-Builder pattern unblocks parallel Builder dispatch when substrate decisions span multiple files** — the architect-side-design phase converts implementation-time ambiguity into design-time ambiguity (much cheaper to resolve). |
| 192B | Diagnostic report PDF export + Share Sheet (feature) | ✅ | **Feature half complete** (192 viewer + 192B PDF/Share = substrate-then-feature pair done). 4 commits both repos: backend Commit 1 (composer preset filter + POST /pdf + deterministic-rendering pytest, 3 xfailed pending F34) + Commit 1.5 (F34 fix: opt-in `deterministic=True` via reportlab `SimpleDocTemplate(invariant=True)` propagation, single-line-fix world per step-zero source audit) + mobile Commit 2 (`react-native-share@12.3.1` install + 5-min compat audit caught + mitigated #1683 Android null-Uri + `usePdfDownload` + `useReportShare` + `shareTempCleanup` per-share-unlink + 24hr startup sweep + typed `PdfDownloadError` discriminated union) + mobile Commit 3 + finalize (Share PDF button on `ReportViewerScreen` adjacent to `SectionToggle` per user mental-model + `share(filePath)` call-time refactor + per-share-unlink-only-on-success refinement + `reportShareErrorCopy` helper for 5-kind user-facing copy register). 34 backend + 72 mobile tests; 435 mobile suite green. **F33 process refinement validated on first use** — plan-time audit caught Phase 182 + Phase 192 substrate already in place; NO v1.0.1 reshape needed. F34 closed; F28 + F29 + F30 reaffirmed deferred. Backend pyproject 0.3.2 → 0.3.4; mobile package 0.1.2 → 0.1.4. |
| 193 | Shop dashboard (mobile) | ✅ | **Mobile-only consumer-side build** on Phase 161/162/164/180 backend substrate (3,400 LoC + 25 endpoints already shipped). Branch `phase-193-shop-dashboard` BOTH repos. **CLAUDE.md Step 0 audit ran first** (F33 promoted to canonical-process pre-plan-v1.0): backend rich, mobile greenfield, subscription/upgrade UI confirmed absent. **Scope**: 4th bottom tab `ShopTab` (tier-reactive, no restart on upgrade), sticky session shop picker (multi-shop), WO list with sort toggle (Newest/Priority/Triage), WO detail with **data-driven section composition** (`WorkOrderSection` discriminated union — load-bearing forward-look architecture mirroring Phase 192's `ReportSection` for 194/195/196 to extend), state transitions (in_progress/on_hold/completed only — cancelled+draft deferred), reassign via member-picker with RBAC-aware default filter. **3 mobile commits planned** (hooks+errors+nav / screens / smoke+finalize). **F-tickets**: F35 NEW (triage explainability), F36 conditional (member workload counts if backend exposes). Plan v1.0 written 2026-05-06. |
| 194 | Camera + photo integration | ✅ | **Substrate half of substrate-then-feature pair** (194 capture/attach/display + 194B AI photo analysis). Branch `phase-194-camera-photo-integration` BOTH repos. **CLAUDE.md Step 0 dual-direction audit ran first** (canonical-process invocation #2 post-Phase 192B promotion): backend has video-route pattern (Phase 191B) + KB photo_annotation (Phase 119, NOT applicable); mobile has `react-native-vision-camera@4.7.3` already installed + `useSessionVideos`/`videoStorageCache`/`VideoCaptureScreen` direct templates. **NO new dependency.** **3 commits**: backend Commit 0 (`71658ac`, 2033 insertions / 10 files — migration 041 + `work_order_photos` table 18 cols + 4 indexes + `wo_photo_repo` + `photo_pipeline.normalize_photo` HEIC/EXIF/resize/JPEG-85 + 6 photos endpoints + 5 error mappings + 44 tests + F9-discipline fix on Phase 192's SCHEMA_VERSION pin); mobile Commit 1 (`c3c6c41`, 4267 insertions / 16 files — substrate: types + builder 4th-param + WorkOrderPhotosSection variant + `_renderPhotos` branch with pairs/standalones/undecided banner + photoStorageCache w/ 7-day sweep + photoCaptureMachine 4-state reducer + PhotoCaptureScreen w/ 4-button classify + useWorkOrderPhotos hook + 76 new tests); mobile Commit 2 (`6d55f81`, 723 insertions / 5 files — WO detail wiring + ClassifyPhotosScreen classify-later surface w/ pair picker + ShopStack registration + App.tsx cold-start sweep + 8-step smoke gate 11 tests). **Section E load-bearing test PASSED**: Phase 193's forward-look commitment held empirically — first variant addition required NO plan amendment, NO photo-data deformation into text-row shape, NO renderer architecture rewrite. F-tickets: F37 watched during execution, no instance #3 surfaced; F38 candidate NOT promoted (smoke gate confirmed existing ShopAccessError 5-kind union sufficient). Schema 40 → 41; backend `pyproject` 0.3.6 → 0.4.0 (+pillow-heif 1.3.0); mobile `package` 0.1.7 → 0.2.0. Backend tests 44 added + 1 fixed; mobile tests 631/631 (was 444; +187 net). **Phase 194B readiness**: AI photo analysis ships on `analysis_state` + `analysis_findings` columns already present in migration 041 — no schema migration needed. |
| 195 | Voice input for symptom description | ✅ | **Substrate-half complete** (195 capture + on-device preview + audio storage + keyword extraction + variant; **195B** ships cloud Whisper + Claude-rich extraction + cost monitoring + VAD). 4 commits backend + 5 commits mobile across 2 days. Schema 41→42 (`voice_transcripts` 25 cols + `extracted_symptoms` 14 cols + 7 indexes); 6-endpoint route under `/v1/shop/{id}/work-orders/{id}/transcripts`; CLI `motodiag transcripts sweep`; backend 0.4.0→0.5.0 (no new dep). Mobile 0.2.0→0.3.0 (+`@react-native-voice/voice` + `react-native-audio-recorder-player`); WorkOrderSection 6→7 variants; 10-module mobile substrate including `audioPlaybackErrors` typed-3-kind union with cache-then-stream-fallback; cold-start audio sweep wired in App.tsx (Mobile 1.5 fix-cycle commit caught by trust-but-verify pre-Mobile-2). **Section E load-bearing test #2 PASSED** — forward-look commitment validated under time-series-with-extracted-output shape (structurally different from Phase 194 photos). **F37 instance #3 surfaced** at Backend Commit 0.5 (Phase 195 transcripts.py response models regressed from Phase 194's PhotoRole Literal pattern); Track 1 fix landed in 0.5 + verified verbatim at Step 9 (TS2322 single-error capture in WorkOrderSectionCard.tsx:542 `_renderExtractionBadge` `never` cast); Track 2 deferred to dedicated phase (likely Phase 195C — lint + retroactive validation + F9 subspecies, same shape as 191D). F38 + F39 NEW filed at plan-write for symptom-storage unification + Phase 96 PCM transcode triggers. 10-step smoke gate (8-step + Steps 9 + 10 added in plan v1.0.2/v1.0.3 amendments); Step 10 four-tier disposition curve pinned in jest, device-side phrase capture deferred to first real-device run post-finalize. Backend 45 tests + mobile 102 tests added (mobile 631→693 net). Plan amended v1.0→v1.0.1→v1.0.2→v1.0.3→v1.1; all amendments architect-side review locks of UX + framing decisions, NOT scope changes. **Substrate-half of substrate-then-feature pair** (195 capture + on-device preview + audio upload + canonical-shape transcript + keyword extraction + variant; **195B** ships cloud Whisper + Claude-rich extraction + cost monitoring + VAD). Branch `phase-195-voice-input` BOTH repos. **CLAUDE.md Step 0 dual-direction audit ran first** (canonical-process invocation #3 post-Phase 192B promotion): backend has Phase 96 audio-capture infrastructure (engine-sound diagnostic — different pipeline, coexists) + Phase 178 symptom substrate (`symptoms` catalog, `engine/symptoms.SYMPTOM_CATEGORIES` keyword dict directly reusable for extraction); NO existing STT/Whisper integration. Mobile has `useCameraPermissions` already tracking microphone (factor out `useMicrophonePermissions` cleanly, no fork) + Phase 191B/194 hook+cache+machine patterns transfer. **TWO new mobile deps** (`@react-native-voice/voice` for on-device STT preview + `react-native-audio-recorder-player` for raw audio capture); **ZERO new backend deps in 195** (`openai>=1.0` for Whisper deferred to 195B). **Scope**: `voice_transcripts` table (22 cols incl. ISO 639-1+region `language` + `preview_text` + `preview_engine` + `extraction_state` enum + 4 substrate-anticipates-feature columns for 195B's whisper_*) + `extracted_symptoms` relational table (12 cols incl. `linked_symptom_id` FK to KB catalog + `extraction_method` enum + `confirmed_by_user_id` for mechanic confirm UX) + `source TEXT NULL` forward-investment column on both (Phase 196 horizon) · `POST /v1/shop/{shop_id}/work-orders/{wo_id}/transcripts` multipart route + per-WO 30 / monthly 200 quotas + 5 sibling endpoints (list, single, PATCH extracted_symptom, DELETE, audio file stream) · backend `audio_pipeline.normalize_audio` (M4A/WAV/Ogg → 16 kHz mono 16-bit PCM canonical) · keyword extraction via existing `engine/symptoms.categorize_symptoms` (sync in-handler; threshold 0.5 default, calibrate against 20-jargon fixture at Backend Commit 0; deviation note if calibration shows different) · 60-day audio sweep substrate (`media/audio_sweep.prune_old_audio`, `motodiag transcripts sweep` CLI; Phase 192B share-temp + Phase 194 cleanupOldPhotos pattern; transcripts permanent, audio bytes only pruned) · mobile `useMicrophonePermissions` factor-out + `audioStorageCache` w/ 7-day cold-start sweep + `audioCaptureMachine` 4-state reducer (idle/recording/uploading/uploaded/upload-failed; APP_BACKGROUNDED→failed) + `VoiceCaptureScreen` w/ button-tap-to-start-and-stop affordance (Section K — VAD deferred to 195B) + on-device STT preview parallel-running with raw audio capture + `useWorkOrderTranscripts` hook + `WorkOrderTranscriptsSection` discriminated-union variant addition (**SECOND test of Phase 193's forward-look commitment** — time-series + extracted-output shape, structurally different from Phase 194's photos; renderer adds 3rd layout idiom = timeline-with-extracted-symptom-chips) + `TranscriptReviewScreen` for mechanic confirm/edit of extracted symptoms (linked_symptom_id picker against KB catalog). **3 commits planned** (backend Commit 0 + mobile Commits 1, 2). **F-tickets**: F37 (value-level enum mismatch) watching during execution — 4 new enums (`language`, `preview_engine`, `extraction_state`, `extraction_method`); instance #3 escalates to file. **F38 NEW filed at plan-write**: unify symptom storage across `diagnostic_sessions.symptoms` (JSON list, Phase 178) + `voice_transcripts.extracted_symptoms` (relational, Phase 195) + future OBD-captured symptoms (Phase 196). Plan v1.0 written 2026-05-06. |
| 195B | Voice symptom — cloud Whisper + Claude-rich extraction | 🚧 | **Feature half of the 195/195B substrate-then-feature pair.** Branch `phase-195B-cloud-whisper` BOTH repos. Substrate-audit-against-post-195-state ran first (CLAUDE.md substrate-feature-pair audit framing, first invocation): schema-zero on the Whisper half (`voice_transcripts.whisper_*` columns + `extraction_state='extracting'` + `extraction_method='claude'` enum values all shipped as substrate-anticipates-feature in migration 042); mobile `_symptomChipStyle` `claude` branch + `_renderExtractionBadge` `extracting` state already shipped Phase 195 Mobile Commit 1. **Step 10 acoustic capture (deferred from Phase 195, ran 2026-05-16 on physical iPhone) PASSED** — worst-case 0.92 across shop-noise + distance conditions; on-device STT held up, so cloud Whisper is reframed extraction-richness-substrate NOT canonicalization-priority — 195B lighter than worst-case planning assumed. **Scope**: OpenAI `whisper-1` cloud transcription (M4A native, no transcode) · Claude-rich extraction via tool-use structured output, `DiagnosticClient.extract_symptoms` extension, Haiku (redirect-to-Sonnet trigger = Commit-0-calibration capability-bound misses) · threshold-gated Claude-fallback, hybrid calibration corpus (Step 10 transcripts + ~15-20 synthetic edge cases), method locked v1.0 / value derived Commit 0 · `cost_events` ledger table (migration 043) + `motodiag costs report` CLI + soft per-shop cap · async BackgroundTasks pipeline (191B Vision precedent; upload→201 `extracting`→Whisper→Claude→`extracted`; **state-flip + row-writes single atomic transaction = Commit 1 acceptance criterion**, refetch-mid-write race test) · F44 port-default 8080→8000 fold-in to Commit 0 config change (conditional on shallow hardcoded-8080 audit; un-fold if deep; openapi.json `:8080` is a known audit hit). **Backend-heavy / mobile-light** (inverse of Phase 195) — substrate-anticipates-feature + already-shipped `claude` chip branch mean mobile surface is small. **3 commits planned**: Backend Commit 0 (Whisper + cost substrate + F44) + Backend Commit 1 (Claude extraction + threshold + async) + Mobile Commit 2 (refetch verification + smoke gate). VAD deferred (Step 10 retired the accuracy mandate; F-ticket the UX argument). F47 NEW (threshold revisit ticketed obligation — re-derive after N real production transcripts). F37 Track 1 Literal discipline carryforward (all new enums Literal day one). Plan v1.0 written 2026-05-16. |
| 195C | F37 Track 2 — Literal-vs-CHECK-constraint lint rule | 🔲 | **RESERVED SLOT** (meta-tooling phase, same precedent as 191B→191C→191D — feature ships, then meta-tooling responds to discovered drift). F37 instance #3 surfaced Phase 195 Backend Commit 0.5; Track 1 fixed in-phase, Track 2 (systematic enforcement) deferred here. Scope when opened: lint rule enforcing "Pydantic response models for fields with DB CHECK constraints must use `Literal[...]` matching constraint values" + retroactive validation across 191B/192/193/194/195/195B backend code + F9 pattern-guide subspecies addition (contract-surface-drift). NOT iOS-parity (that landed as a CLAUDE.md PR-review checklist item per F40-refined, deliberately not lint scope — cross-platform permission parity is a semantic cross-file inference, not a parseable property). Opens post-195B-finalize. |
