...
> **v1.1 amends v1.0 after D1 halted the phase.** The amendments are at the
> end of this document and D2.6 is revised in place. v1.0's text above is
> left as written, including the D1 step that specified the `if` field that
> turned out not to work — the plan is a record as well as an instruction,
> and silently correcting a step hides that the check caught something.
...
### D2.6 The hook — REVISED at v1.1, because D1 proved this wrong
...
**The `if` field is gone.** D1 measured that `PreToolUse` ignores it in this
build — see v1.1-2 — so the guard **filters itself**: it reads the tool call
as JSON on stdin, takes `tool_input.command`, and **exits 0 immediately
unless that command is a `git push`.** Only then does it run the closeout
test and exit 2 on failure.

This is correct whether or not `if` is ever honoured, and it does not depend
on a field whose behaviour differs between two events of the same mechanism.
...
# v1.1 amendments (2026-09-22, after D1 halted the phase)

D1 ran the mechanism before anything was built, and three of its results
change the plan. Operator accepted all four points below on 2026-09-22.

## v1.1-1. Skills are discovered at session start, from the session root

**Rule adopted: moto-diag work happens in a session rooted at the
repository.** No symlinks, and no `.claude/` at the parent directory —
both were considered and rejected as workarounds that hide where the
mechanism is actually reading from.

Two consequences:

* `moto-diag/.claude/skills/` is the correct and only location.
* **A skill added during a session is not invocable in that session.** Any
  plan step that says "write a skill, then use it" is wrong by
  construction. Skills are written, committed, and used from the *next*
  session.

**Recorded where it will be read:** the repo `README` and the project
`CLAUDE.md` (which does not exist yet — it arrives with D5's split) both
state that moto-diag sessions are started at the repository root and why.

**Still open, and it does not block the build.** `/ping` has not resolved in
any session yet. This session created the file, so its failure is exactly
what start-time enumeration predicts and settles nothing further. **One
check in a fresh repo-rooted session decides it**, and `ping/SKILL.md` is
committed as that fixture. The build does not depend on the answer, because
the enforcement is the test and the hook — neither of which needs a skill to
be invocable.

## v1.1-2. `PreToolUse` ignores `if` in this build — with the evidence

Isolated using **non-blocking marker hooks**, so a wrong result could not
lock out `Bash` a second time. Each hook appended a line to a file; a
non-`git push` command was then run.

| hook | `if` | non-push command run | marker |
|---|---|---|---|
| `PostToolUse` on `Bash` | `Bash(git push *)` | once | **absent** — filtered correctly |
| `PreToolUse` on `Bash` | `Bash(git push *)` | twice | **1 → 2** — fired on both |

The same field, the same value, the same matcher, opposite behaviour on the
two events. **v1.0's D2.6 specified precisely the combination that does not
filter**, which would have made the push guard block every `Bash` command in
the session. It did, for one turn, which is how it was found.

D2.6 is revised above: no `if`, and the guard filters itself from
`tool_input.command` on stdin.

## v1.1-3. The guard must never block a non-push command — and this is tested

The lockout was not a nuisance; it is the failure mode of this whole design.
A guard on `Bash` that is wrong about *which* commands it guards takes away
the tool needed to remove it.

* **The guard exits 0 for anything that is not a `git push`**, before it does
  any work at all — no test run, no file reads, no chance to fail open or
  closed by accident.
* **A test asserts it**: `pre_push_guard.sh` is invoked directly with a
  synthetic stdin payload for a non-push command (`ls -la`, and a command
  whose *text contains* the words "git push" inside a quoted string, which
  must also pass), and must exit 0 in every case. It is then invoked with a
  real `git push` payload against a known-bad phase and must exit 2.
* **The README documents recovery**: if a hook ever blocks `Bash`, edit
  `.claude/settings.json` with the **Write tool**, which does not pass
  through a `Bash` matcher. This is written down because the recovery is not
  obvious at the moment it is needed — every instinct is to reach for a
  shell.

## v1.1-4. Config loads with roughly one turn of delay

A hook written to `settings.json` does **not** take effect for the tool calls
in the same turn. D1's first `git push` after installing a blocking guard
went through untouched; the guard fired on the following turn.

**Consequence for this phase and for any later one: a hook test waits a turn
before asserting.** A guard verified in the turn that installed it will look
like it does not work, and the natural next move — "the config must be
wrong, change it" — makes the problem worse. Recorded in the skill's own
`CHANGELOG.md` as well as here, because that is where the next person
changing the hook will be reading.

## What v1.1 does not change

D2.4's seven artefact assertions, D2.5's hand-written known-bad fixture,
D2.3's single implementation of the word count, and the order
closeout → finding → refute are all unchanged. The enforcement design —
