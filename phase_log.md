...
### 2026-04-27 — Phase 189 complete — Diagnostic session UI (mobile) + bottom-tab nav (7 commits, gate round 1 GREEN)

**Track I phase 5 of 20 complete (185 / 186 / 187 / 188 / 189).** Code shipped in `Kubanjaze/moto-diag-mobile`; phase docs live here in `moto-diag/docs/phases/completed/189_*.md`. Branch `phase-189-diagnostic-session-ui`, **7 commits** rebase-merged to `main` at finalize, branch deleted local + remote.

**Roadmap swap at plan time:** Phase 189 was originally "DTC code lookup screen" and Phase 190 was "Interactive diagnostic session." Swapped because the session UI is the load-bearing mechanic workflow — DTC lookup is a sub-surface most useful inside a session. Phase 190 now picks up DTC lookup as a standalone screen + tap-from-SessionDetail integration. Both ROADMAP rows updated in the plan commit (`f2b25b9`).

**Shipped (mobile repo, not this backend):**

- **3 new screens.** `SessionsListScreen` (FlatList + RefreshControl + empty-state CTA + monthly-quota footer using session-shaped keys + per-row StatusBadge open/in_progress/closed). `SessionDetailScreen` (six cards — Vehicle / Symptoms / Fault codes / Diagnosis / Notes / Lifecycle, each list card with always-visible inline append input, Diagnosis card toggles view↔edit). `NewSessionScreen` (freehand make/model/year + tap-to-fill from useVehicles-backed garage list with Unlink escape hatch + optional initial symptoms multiline + initial DTCs comma-separated uppercased on submit).
- **2 new hooks.** `useSessions` (mirrors useVehicles shape; uses session-shaped quota keys `total_this_month` / `monthly_quota_limit` / `monthly_quota_remaining` — NOT vehicles' parity; explicit guard test for the difference). `useSession(id)` (single fetch + path-param type-enforced).
- **First bottom-tab nav.** RootNavigator rewritten as `createBottomTabNavigator` with three tabs (HomeTab / GarageTab / SessionsTab); each tab is its own native-stack so back-nav within a tab is independent (open VehicleDetail in Garage, switch to Sessions, switch back → still on VehicleDetail). One new runtime dep: `@react-navigation/bottom-tabs@7.15.10` (matches existing `@react-navigation/native@7.2.2`; pure-JS, no new native modules; no patch-package needed).
- **F1 follow-up resolved as Commit 1.** battery_chemistry was free-text in Phase 188; now `SelectField<BatteryChemistryLiteral>` with `nullable allowNull` (closed enum 5 values: li_ion / lfp / nmc / nca / lead_acid). `BatteryChemistryLiteral` manually defined in `src/types/api.ts` because backend exposes the field as bare `Optional[str]` even though the route handler enforces the enum at the boundary.
- **`SelectField` extended.** Discriminated-union `nullable` discriminator. Existing closed-required call sites (protocol/powertrain/engine_type) unchanged. New nullable variant adds opt-in `allowNull` + `allowCustom`. Pure helpers `buildSelectRows` + `getTriggerDisplay` exported for unit tests. Used for both battery_chemistry (allowCustom off) and severity (allowCustom on).
- **Severity round-trip per pre-Commit-5 sketch sign-off.** `src/types/sessionEnums.ts` ships SeverityLiteral + SEVERITY_OPTIONS + SEVERITY_LABELS + 3 helpers — `deriveSeverityState` / `packSeverityForSubmit` / `renderSeverityForView`. State machine: `severityChoice: SeverityLiteral | null` + `severityCustom: string`, invariant never both populated. Option 1 (transient `customInputVisible` flag; trigger reads "—" until first keystroke; custom Field appears focused below). `Field.tsx` converted to `forwardRef<TextInput, Props>` to support the auto-focus.
- **8 distinct POSTs across the phase** (POST /v1/sessions create + POST /symptoms / /fault-codes / /notes appends + POST /close + /reopen lifecycle + PATCH for diagnosis). 2 new `client.test.ts` regression-guards: Content-Type + X-API-Key on POST /symptoms (body-bearing); X-API-Key + Accept + correct path-param URL on POST /close (empty-body).

**Architect-gate round 1 PASSED — zero fix commits required.** Phase 188 set the precedent of "round 1 BLOCKED + 3 fix commits"; Phase 189 inverted it. Two factors: (1) Phase 188 commits 6/7/8 transport guards covered the new POST surfaces in advance; (2) the pre-Commit-5 sketch sign-off for severity Other… caught design ambiguity (Option 1 vs 2) at sketch-cost instead of implementation-cost.

**Verified:** Cold gradle rebuild clean. All 20 smoke-flow steps green: F1 fix verified (battery_chemistry dropdown + prettified label); session create + nav + quota footer using session-shaped keys; three append POSTs (symptoms ×2, DTC with lowercase→uppercase normalization, note with auto-timestamp prefix); severity round-trip across closed-pick → Other… → closed-after-custom with re-entry pre-population proof; lifecycle close/reopen with empty-body POST regression checks; bottom-tab state preservation; cold relaunch full state hydration. No Phase 186 BLE / Phase 187 auth / Phase 188 vehicle CRUD regressions. **162 / 162 Jest tests green** (90 baseline + 72 new — 18 SelectField helpers + 10 vehicleEnums battery_chemistry + 8 useSessions + 5 useSession + 9 NewSessionScreen pack helpers + 20 sessionEnums helpers + 2 client transport guards). `tsc --noEmit` clean.

**Architect sign-off:** GATE PASSED on round 1. Two non-blocking follow-ups filed in `moto-diag-mobile/docs/FOLLOWUPS.md`: **F2** — per-entry edit/delete on open sessions (smoke testing surfaced the demand for typo correction; defensible middle ground: open sessions → entries can be edited/deleted, closed sessions → immutable; backend likely needs a `deleted_at` soft-delete column; recommended target Phase 191). **F3** — lifecycle audit history (Closed timestamp vanishes from Lifecycle card on Reopen, reflecting pure current state; persisting close/reopen events as a timeline is generally more useful for forensic-style diagnostic logs; product call, not a bug; recommended target Phase 191 follow-up). F1 from Phase 188 marked closed (resolved by Phase 189 Commit 1).

**Backend repo impact:** zero code change. **All Phase 189 docs live in this backend repo's `docs/phases/completed/`** per project convention.

Mobile package version 0.0.3 → 0.0.4. Mobile project `implementation.md` 0.0.5 → 0.0.6. Backend project `implementation.md` 0.13.4 → 0.13.5.

**Key finding:** **when an architect-gate sketch sign-off lands BEFORE the implementation, round 1 holds.** Phase 188 had no pre-implementation review for vehicle CRUD and the gate caught 2 transport bugs + 1 cosmetic nit (3 fix commits). Phase 189 paused before Commit 5 to sketch the severity Other… round-trip UX in writing — the State A/B/C model, the `deriveSeverityState` / `packSeverityForSubmit` / `renderSeverityForView` trio, the Option 1 vs Option 2 trigger-display question, the agreed `customLabel: 'Other'`. The user spotted a real inconsistency in Step 5 wording before any code existed; resolution was a 1-line update to the smoke-flow doc. Round 1 then passed clean. Two takeaways for the rest of Track I: (1) every commit that introduces a non-obvious state machine (severity round-trip, optimistic update layer, offline op-queue) gets a pre-implementation sketch posted for sign-off, not just an implementation.md plan section; (2) round 1 GREEN this phase is repeatable — pre-implementation sketches let the user catch design issues at sketch-cost (~30 min) instead of implementation-cost (~3-4 fix commits).

Next: **Phase 190 — DTC code lookup screen** (search by code or text, voice input deferred to Phase 195, offline DTC database from Phase 198, plus tap-to-lookup integration into SessionDetail's fault-codes list per the Phase 189 scope swap).

---

### 2026-04-28 — Phase 190 complete — DTC code lookup screen + cross-link from SessionDetail (7 mobile + 1 backend commits, gate round 2 GREEN)

**Track I phase 6 of 20 complete (185 / 186 / 187 / 188 / 189 / 190).** Code shipped in `Kubanjaze/moto-diag-mobile` (mobile) + 1 backend fix on `Kubanjaze/moto-diag` master. Phase docs live here in `moto-diag/docs/phases/completed/190_*.md` per project convention. Mobile branch `phase-190-dtc-code-lookup-screen`, **7 commits** (5 build + 2 fix) rebase-merged to mobile `main` at finalize. Branch deleted local; remote was never pushed per Phase 188/189 precedent.

**Roadmap context:** Phase 190 was originally "DTC code lookup screen" (was Phase 177); swapped with Phase 189 ("Interactive diagnostic session" was 178) at Phase 189 plan time so the canonical mechanic workflow shipped first. Phase 190 picked up DTC lookup as a standalone screen + the cross-link integration into Phase 189's SessionDetail (which explicitly carried "raw P0171 codes only" as a known gap).

**Shipped (mobile repo, code only):**

- **2 new screens.** `DTCSearchScreen` registered in HomeStack only — general-lookup launcher with search-as-you-type (300ms debounce, race-cancellation via requestId counter, 6 render states: empty-query prompt / loading / success-with-results / no-results / error / debouncing). `DTCDetailScreen` registered in BOTH HomeStack AND SessionsStack via cross-stack same-route-name pattern; `sourceSessionId` render-hint param drives the "Opened from session #N" footer when reached via fault-code tap.
- **2 new hooks.** `useDTCSearch` with the debounced + race-cancellation contract (10-test suite including a deterministic race test that resolves promises out of order to verify stale responses are dropped); `useDTC(code)` for single-DTC fetch (mirrors useSession shape).
- **HomeScreen integration.** New "DTC lookup" Section between Authed-smoke and BLE — single button → `navigation.navigate('DTCSearch')`.
- **SessionDetailScreen integration (the cross-link).** Fault-code list rows became `<TouchableOpacity>` with chevron `›` affordance → push to DTCDetail within SessionsStack with `{code, sourceSessionId}` params. Closes Phase 189's "raw P0171 only" known gap.
- **Severity rendering reuse.** DTCDetailScreen + DTCSearchScreen import `SEVERITY_OPTIONS` + `SEVERITY_LABELS` + `renderSeverityForView` from Phase 189's `sessionEnums.ts`. 2-line top comment in `sessionEnums.ts` documents the cross-use without renaming the file (per Phase 190 plan sign-off: file-name-vs-content drift accepted over renaming churn).

**Backend fix (commit 8, `3d3e7ab` on master):** `data/dtc_codes/generic.json` 20 → 35 codes (real mechanic-grade content per row — P0521 includes a HARD WARNING in the fix_summary about verifying actual oil pressure with a mechanical gauge before assuming sensor failure); `load_dtc_file` pre-deletes existing `(code, make)` rows so re-seeding is idempotent for NULL-make rows that SQLite's `UNIQUE(code, make)` constraint can't enforce. Existing dev DBs self-clean their accumulated dups on the next re-seed via this pre-delete.

**Architect-gate ROUND 1 FAILED at Step 11 with 3 must-fix bugs.** Same shape as Phase 188 round 1 but different specific failures:

- **Bug 1**: DTCSearchScreen FlatList `keyExtractor` used `item.code` alone; backend search returned multiple rows with the same code (legacy NULL-make duplicates from re-seeded generic.json — Bug 3a) → React "two children with same key" warning + 7+ visually identical P0100 rows in results.
- **Bug 2**: Tapping P0171 fault-code in SessionDetail → DTCDetailScreen rendered "Couldn't load DTC: [object Object]". Same flavor as Phase 188 Bug 2 (HVE unwrap), different shape: KB endpoint uses FastAPI's stock `raise HTTPException(404, detail=...)` returning `{detail: string}` — NOT Phase 175's ProblemDetail envelope. `isProblemDetail` returned false; `describeError` fell through to `String(err)` → `[object Object]`. The substring-match-on-error-text discriminator pattern in DTCDetailScreen compounded the failure: with "[object Object]" as the message, the "not_found" branch was unreachable.
- **Bug 3** split: 3a — SQLite's `UNIQUE(code, make)` constraint doesn't enforce uniqueness when `make IS NULL` (NULL ≠ NULL in UNIQUE semantics); each `motodiag db init` re-seeded generic catalog rows, accumulating duplicate generic rows. Architect's emulator DB had 7+ identical P0100 rows. 3b — `data/dtc_codes/generic.json` shipped 20 codes total; P0171 (literally cited in Phase 189's smoke spec) + the architect's "top 20 most-common" set (P0171/P0172/P0174/P0175 fuel-trim, P0299 boost, P0302-P0304 misfire extension, P0430, P0440/P0442/P0455 EVAP, P0506/P0507 idle, P0521 oil pressure) were genuinely missing.

**3 fix commits**:

- **Commit 6 (mobile, `d028445`)** — Bug 1: composite keyExtractor `${index}-${code}-${make ?? 'generic'}` extracted to new `src/screens/dtcSearchHelpers.ts` module + 6 regression tests including the 7-identical-code-rows scenario.
- **Commit 7 (mobile, `744becf`)** — Bug 2: typed-error refactor. New `src/hooks/dtcErrors.ts` with `DTCError` discriminated union (`not_found | server | network | unknown`) + `classifyDTCError(args)` reading `response.status` + `extractErrorMessage(err)` handling BOTH Phase 175 envelope AND FastAPI default `{detail: string}`. DTCDetailScreen switches on `error.kind` via Record map; `not_found` UX shows "DTC code not found" + spelling-check hint + Back-only (no Retry per spec). 27 new regression tests paralleling Phase 188 commit-7's 17 HVE tests including an explicit "no [object Object]" guard. Broken commit-1 useDTC test (which mocked the wrong Phase 175 envelope shape — never matched the real backend) replaced with the actual FastAPI shape + new 500/network coverage.
- **Commit 8 (backend, `3d3e7ab` on master)** — Bug 3: seed expansion 20 → 35 codes + idempotent `load_dtc_file` + 3 regression tests covering re-seed idempotency + dedup-existing-dups simulation.

**Architect-gate ROUND 2 PASSED on all 7 re-smoke verifications** (after `motodiag db init` re-run on emulator backend cleaned the accumulated dups + loaded the expanded catalog): P0171 happy path with full diagnostic content (Medium badge, 5 common causes, fix summary with PSI ranges, "Opened from session #1" footer); no [object Object] anywhere on screen; BOGUS123 not_found path showing the correct UX (title "DTC code not found", server-supplied "DTC code 'BOGUS123' not found" message, spelling-check hint, Back-only); back-button stack correctness across the cross-stack same-route-name pattern; tab-state preservation; P0420 happy path with distinct content from P0171 (different category/causes, confirms not template clones); cold relaunch persistence with full Session #1 hydration including round-2 fault-code appends.

No Phase 186 BLE / Phase 187 auth / Phase 188 vehicle CRUD / Phase 189 session CRUD regressions surfaced in either round.

**Verified:** **210 / 210 mobile Jest tests green** (162 baseline + 48 new across all commits — 5 useDTC build + 10 useDTCSearch build + 6 dtcResultKey fix + 27 dtcErrors fix + 2 useDTC fix-additions − 1 useDTC test rewritten); `tsc --noEmit` clean every commit. Backend Phase 5 DTC tests 17/17 + targeted KB regression (Phase 9 + 111 + 128 + 179 + Gate 1) 97/97 green.

**Architect sign-off:** GATE PASSED on round 2. **Five Phase 191 polish follow-ups filed in `moto-diag-mobile/docs/FOLLOWUPS.md`:**
- **F2** — per-entry edit/delete on open sessions (carry-over from Phase 189; recommended target Phase 191).
- **F3** — lifecycle audit history (carry-over from Phase 189; recommended target Phase 191).
- **F4** — make/family chip on DTCSearch result rows (new from Phase 190 round-2 smoke; orthogonal to Bug 1's keying fix — visual disambiguation for legitimate same-code multi-make variants).
- **F5** — "Code not in catalog yet" empty-state copy (new from Phase 190; surfaced when architect typed P0101/P0102 and saw the same generic "No DTCs match" copy as a typo query).
- **F6** — `useDTC` memoization to suppress React 18 StrictMode dev-only double-fetch on mount (cosmetic; production builds run a single fetch).

**Backend repo impact:** 1 fix commit (loader idempotency + seed expansion + 3 new tests) — minor scope increase from the plan's "zero backend changes" assumption. Schema unchanged at v38.

Mobile package version 0.0.4 → 0.0.5. Mobile project `implementation.md` 0.0.6 → 0.0.7. Backend project `implementation.md` 0.13.5 → 0.13.6.

**Key finding:** **mock fidelity is a load-bearing concern, not a polish concern.** Phase 189's lesson "pre-implementation sketches catch design issues at sketch-cost (~30 min) instead of implementation-cost (~3-4 fix commits)" applied to design ambiguity (severity Other... state machine). It does NOT generalize to the failure modes Phase 190 hit at round 1. Bug 2's root cause was a test mock that didn't match the real backend wire format: the test passed because it mocked a 404 with the Phase 175 envelope shape (`{title, status, detail}`), but the KB endpoint actually returns FastAPI's stock `{detail: string}`. The substring-match-on-error-text discriminator pattern compounded the failure. **Two takeaways for the rest of Track I:**

1. Every error-rendering test that asserts on an error-shape should anchor that shape to a fixture pulled from a real backend response OR to the OpenAPI spec contract — never to the test author's assumption about what the backend returns. The Phase 188 Bug 1 (Content-Type stripped on POST) and Phase 190 Bug 2 (404 shape misclassified) are the same class of bug: mobile test layer claimed coverage, test passed, path was actually broken in production.
2. Replace fragile string-matching with discriminated-union types whenever the discriminator is an HTTP status code or a backend-controlled enum — the type system catches drift; substring matches don't.

Next: **Phase 191 — Video diagnostic capture (mobile)** per ROADMAP — film bike running, auto-extract audio + key frames → AI analysis. The 5 polish items (F2-F6) may be folded into Phase 191 if the touched code overlaps, or filed as a standalone polish phase if it doesn't.

---

### 2026-04-29 — Phase 191 complete — Video diagnostic capture (mobile, capture-only substrate; 8 commits, gate round 2 GREEN; Phase 191B added as new ROADMAP row)

**Track I phase 7 of 21 complete (185 / 186 / 187 / 188 / 189 / 190 / 191).** Track count grows from 20 to 21 because Phase 191B is added as a NEW ROADMAP row at this finalize per Kerwyn's pre-plan ask. Code shipped in `Kubanjaze/moto-diag-mobile`; phase docs live here in `moto-diag/docs/phases/completed/191_*.md` per project convention. Mobile branch `phase-191-video-diagnostic-capture`, **8 commits** rebase-merged to mobile `main` at finalize (6 build + 1 in-cycle Commit-3 fix + 1 full-gate fix). Branch deleted local; remote was never pushed per Phase 188/189/190 precedent.

**Scope split locked at plan:** original ROADMAP wording for Phase 191 was "film bike running, auto-extract audio + key frames → AI analysis" — full scope is honestly 3-4 phases compressed into one. Split into 191 (mobile capture-only substrate) + 191B (backend upload + Claude Vision AI analysis pipeline) preserves the substrate-then-feature pattern that already worked for Phase 187 → 188 (auth substrate, then CRUD over it) and Phase 189 → 190 (session substrate, then DTC integration). **Phase 191B added as a NEW row in `docs/ROADMAP.md` immediately after Phase 191** — not buried in FOLLOWUPS. Phases 192-204 row numbers stay the same; 191B is a new row, not a renumbering.

**Shipped (mobile repo, code only):**

- **3 new runtime deps** with native code: `react-native-vision-camera@4.7.3` (CameraX on Android, AVFoundation on iOS, H.264/MP4 codec) + `react-native-fs@2.20.0` (DocumentDirectoryPath access — app-private storage, no media perms needed) + `react-native-video@6.19.2` (ExoPlayer-backed playback). **Plan's "Phase 186 redux" risk projection did NOT materialize** — all three built clean against `newArchEnabled=false` on first cold-gradle pass; zero patch-package work needed. Existing `patches/` scaffolding from Phase 187 absorbed the most-likely friction shape as a sunk cost.
- **2 new screens** registered on SessionsStack: `VideoCaptureScreen` (full capture surface — Camera component + reducer-driven state machine + 4 failed-state UIs routed by `error.kind`); `VideoPlaybackScreen` (rn-video v6 with built-in controls + meta band + orange "⏸ Paused" badge for interrupted videos + delete-with-confirm).
- **2 new hooks**: `useCameraPermissions` (Camera + Microphone permission flow + Android 13+ split-perms callout — Phase 191 only requests CAMERA + RECORD_AUDIO since files stay in app-private storage); `useSessionVideos(sessionId)` — **backend-agnostic** Phase 191B handoff contract returning `{videos, addRecording, deleteVideo, refresh, atCap, capReason, isLoading, error}`. Phase 191B will swap implementation from FS-backed to HTTP-backed; consumers (SessionDetailScreen's VideosCard, VideoCaptureScreen save flow) see the SAME hook shape in both phases.
- **3 new pure-helper modules**: `videoCaptureMachine.ts` (5-state reducer `idle | recording | stopping | saved | failed` with Kerwyn fold #1 auto-keep-on-background from saved + Kerwyn fold #2 RecordingError discriminated union `storage_full | permission_lost | codec_error | unknown`); `videoStorage.ts` (RNFS-backed file-system policy: MAX_VIDEOS_PER_SESSION=5, MAX_BYTES_PER_SESSION=500MB, MIN_FREE_BYTES=100MB; `videos/session-{N}/session-{N}-{ISO8601-compact}-{8charuuid}.mp4` + JSON sidecar; `saveRecording` move-not-copy with EXDEV cross-volume fallback + post-move RNFS.stat for fileSizeBytes; `cleanupOrphanedVideos` walks live-set diff); `videoCaptureHelpers.ts` (formatElapsed mm:ss + formatFileSize auto-unit-switching B/KB/MB/GB with one-decimal precision below 10MB + generateShortId 16-char hex + classifyVisionCameraError mapping vision-camera codes to RecordingError kinds).
- **SessionDetailScreen integration**: new VideosCard placed BETWEEN FAULT CODES and DIAGNOSIS (per Kerwyn pre-plan placement: evidence precedes diagnosis); closed-session lockdown HIDES the Record button (not just disables) when `session.status === 'closed'` — symmetric gap flagged for Phase 189's symptoms/fault-codes/notes append inputs filed as F7. SessionsListScreen wires `cleanupOrphanedVideos(liveIds)` in a separate useEffect watching `[sessions, isLoading, error]`, sweeping orphan `session-N/` directories whenever the live session set updates.

**Two architect-gate stops this phase** (per the v1.0 plan's two-stage gate cadence):

**Micro-gate after Commit 1 (PASSED, 2026-04-28):** cold gradle build green on first try, permission-prompt smoke verified, cold-relaunch persistence verified, Phase 186-190 zero-regression smoke green. Filed F8 polish (formatFileSize unit-switching).

**Commit 3 architect-smoke FAILED at Verification 5** with the closure-state-capture bug. Orange "⏸ Paused at 0:XX" badge wasn't rendering on saved-preview after AppState background-during-recording. **Root cause**: closure-state capture. The `onRecordingFinished` closure registered with `cameraRef.startRecording` captured `state` at moment-of-tap-record (`state=idle`), so `wasInterrupted = state.kind === 'stopping' && state.reason === 'interrupted'` always evaluated false. **NOT a reducer bug** — the reducer correctly transitioned to `stopping` with `reason='interrupted'` on the AppState handler's dispatch. The bug was that the callback couldn't see that updated state because it was looking at its captured snapshot from registration-time. Fix (Commit 3 follow-up `ffa383c`): explicit `interruptedRef = useRef<boolean>(false)` set true in AppState background handler BEFORE `stopRecording()`, set false at start of every recording (handleStartRecording) and at user-initiated stop (handleStopRecording), read via `interruptedRef.current` inside `onRecordingFinished` instead of reading `state.kind`. Folded F8 (formatFileSize unit-switching) into the same commit. **Re-smoke PASSED.** Filed F9 polish (document the `useRef`-not-state pattern for callbacks registered with native modules; surfaced from this fix + the meta-observation that this is now the third instance of the "snapshot/assumption doesn't match runtime" failure family on Track I — Phase 188 HVE shape mock, Phase 190 substring-match-on-error-text, Phase 191 closure-state capture).

**Architect full gate after Commit 6 (FAILED, 2026-04-29) with 3 bugs.** 17 of 17 substrate verifications passed; **3 bugs found** in cross-cutting integration:

- **Bug 1 (HIGH) — VideosCard refresh on focus.** SessionDetail → VideoCapture (record + save) → back: VideosCard didn't show the freshly-saved recording until cold restart. Same on the delete return-path from VideoPlaybackScreen. Root cause: SessionDetail doesn't unmount when a sibling screen pushes onto the SessionsStack; useSessionVideos's mount-time effect therefore doesn't re-fire when the screen is revealed again. The hook DID expose `refresh`, but nothing was calling it. **This is the fourth instance of the "snapshot/assumption doesn't match runtime" failure family** — the hook's mount-time snapshot doesn't match the runtime reality of the screen lifecycle.
- **Bug 2 (LOW) — closed-session × has-videos visual hole.** Closed sessions with ≥1 saved video showed playback rows but a visually blank slot below where the Record button used to live. No copy explaining why the capture path was gone.
...
**3-bug fix landed as a single commit (`39948c1`)** with explicit per-bug callouts in the message body for targeted re-smoke verification — small enough that splitting per-bug would add no review value:

- **Bug 1 fix** — `useFocusEffect(useCallback(() => { void refresh(); }, [refresh]))` placed INSIDE VideosCard (not on SessionDetailScreen) so `refresh` is in scope. React Navigation's useFocusEffect fires for any component nested inside the focused screen.
- **Bug 2 fix** — closed branch in VideosCard now renders a cap-pane-shaped block when `videos.length > 0` with cream/amber styling matching the at-cap pane + "Session closed · Reopen this session to record more." copy.
- **Bug 3 fix** — `tabBarIcon: () => null` on `RootNavigator` `screenOptions`. Restores the text-label-only look from Phase 189's design intent. Commit message explicitly notes this was always-there default behavior, not a Phase 191 regression.

**Architect full gate fix-cycle re-smoke PASSED (2026-04-29) on 8 / 8 verifications** across all three bugs (save-return refresh, delete-return refresh, closed-session lockdown copy, wireframe-nav fix) plus full sanity sweep (Garage, Sessions, DTC P0171 happy path, **2:20 PM Paused-badge artifact preserved on Session #1** as 191B regression-coverage fixture per Kerwyn's instruction).

No Phase 186 BLE / Phase 187 auth / Phase 188 garage / Phase 189 sessions / Phase 190 DTC regressions surfaced in either round.

**Verified:** **301 / 301 mobile Jest tests green** (210 baseline + 95 new — videoCaptureMachine reducer tests with all transitions including auto-keep-on-background per Kerwyn fold #1 + videoStorage path math + EXDEV cross-volume save fallback + cap evaluation + orphan cleanup + videoCaptureHelpers formatFileSize unit-switching + classifyVisionCameraError + dtcErrors carry-forward + useSessionVideos contract including the Phase 191B handoff regression guard + useCameraPermissions); `tsc --noEmit` clean every commit.

**Architect sign-off:** GATE PASSED on round 2 fix-cycle re-smoke. **Phase 192+ polish backlog filed in `moto-diag-mobile/docs/FOLLOWUPS.md`** (F8 shipped in Commit 3 — not retained):
...
- **F7** (NEW) — symmetric closed-session lockdown for Phase 189's symptoms / fault-codes / notes append inputs. Phase 191 closed the gap for videos but left it open for the other lists.
- **F9** (NEW) — document the `useRef`-not-state pattern for callbacks registered with native modules + generalized lint rule for the broader pattern. Surfaced from Commit 3 closure-state-capture bug + the meta-observation that this is the third instance of the "snapshot/assumption doesn't match runtime" failure family across Track I.

**Backend repo impact: zero code change** (pure mobile phase). Schema unchanged at v38.

Mobile package version 0.0.5 → 0.0.6. Mobile project `implementation.md` 0.0.7 → 0.0.8. Backend project `implementation.md` 0.13.6 → 0.13.7.

**Key finding: substrate-then-feature splits keep gates gate-sized.** The original Phase 191 ROADMAP scope ("film bike running, auto-extract audio + key frames → AI analysis") was honestly 3-4 phases compressed into one. Splitting into 191 (mobile capture-only substrate) + 191B (backend upload + Claude Vision pipeline) preserved the substrate-then-feature pattern that already worked for Phase 187 → 188 + Phase 189 → 190. **Two takeaways for the rest of Track I:**

1. At plan-write time, count the number of distinct architectural concerns in the proposed scope. If it's > 2, split BEFORE plan v1.0 lands. Phase 191's split avoided a 3-stage smoke gate (capture + upload + AI), each of which would have its own failure modes.
2. The closure-state-capture bug from Commit 3 + the architect-gate Bug 1 (refresh on focus) are the third and fourth instances of the "snapshot/assumption doesn't match runtime" failure family on Track I. Phase 188 (HVE shape mock), Phase 190 (substring-match on error text), Phase 191 Commit 3 (closure captured state at registration-time), Phase 191 full gate Bug 1 (mount-time snapshot didn't match screen lifecycle reality). The pattern is robust enough now that F9 (the useRef-not-state polish item) plus a generalized lint rule are worth filing for Phase 192+ pickup; the family will not stop generating bugs without an architectural intervention.

Next: **Phase 191B — Video diagnostic upload + Claude Vision AI analysis pipeline** (NEW ROADMAP row) — backend `/v1/videos/*` endpoints + ffmpeg frame extraction + Claude Vision call wired through the new HTTP layer for the first time on Track I + `useSessionVideos` hook swap from FS-backed to backend-backed with the consumer surface unchanged. Phase 191's substrate is the foundation; Phase 191B is the feature that justifies the substrate's existence.

---

### 2026-05-04 — Phase 191B complete — Video diagnostic upload + Claude Vision AI analysis pipeline (8 backend + 3 mobile commits across 5 architect-gate rounds; PASS on round 5 / 22 of 22 steps)

**Track I phase 8 of 21 complete (185 / 186 / 187 / 188 / 189 / 190 / 191 / 191B).** The substrate-then-feature pair completion. Phase 191 shipped the mobile capture substrate; Phase 191B is the backend pipeline + AI analysis = the feature that justifies the substrate's existence. **First production wiring of Claude Vision + ffmpeg into the HTTP layer on Track I** — Phase 100-103 (the Claude API learning series + Track C2 media intelligence) shipped as standalone scripts; Phase 191B brings them into the production API surface for the first time.

**Architecture path: 4 build commits + 4 fix-cycles + Commit 7 finalize across 5 architect-gate rounds.**

Commit 1 (`5970aff`) Builder-A in parallel with Commit 2 (`16512a4`) Builder-B per file-disjoint analysis: migration v39 + Video models + `core/video_repo.py` mirroring `session_repo.py` + `media/ffmpeg.py` subprocess wrapper + `engine/client.py:DiagnosticClient.ask_with_images()` extension; Vision pipeline + analysis worker + synthetic Anthropic fixtures. Commit 3-5 collapsed (`c1a6fb3`) Builder-C per "no architect smoke between" judgment: 5 nested REST endpoints + python-multipart install gotcha (filed for Commit 7). Commit 6 (`c2b912c`) Builder-D mobile hook swap with load-bearing assertion: 10 of 12 useSessionVideos.test.ts it() titles preserved verbatim. Architect-gate ROUND 1 BLOCKED at Step 1B → fix-cycle-1 commit `832579d` (motodiag serve startup migration apply + video_repo timestamp format). Architect-gate ROUND 2 BLOCKED at Step 6 → fix-cycle-2 commit `7e9702e` (file:// prefix on FormData multipart). Architect-gate ROUND 3 BLOCKED at Step 5 by tier enforcement working as designed → fix-cycle-3 commit `0babc55` (`motodiag subscription set` CLI + F14 disclaimer disambiguation). Architect-gate ROUND 4 BLOCKED at Step 7 by hardcoded fabricated Vision model ID → fix-cycle-4 commit `c453872` (sonnet alias → `claude-sonnet-4-6` + 14 hardcoded test refs scrubbed + F15 regression guard + MOTODIAG_VISION_MODEL env var override). Commit 7 finalize (this commit): `_parse_probe_stderr` regex fix for ffmpeg 8.1 codec-tag-vs-resolution collision + sample_3sec.mp4 fixture generation + 5 gated tests un-skipped + version bumps + ROADMAP mark + doc updates + rebase-merge to master/main on both repos.

**Architect-gate ROUND 5 PASS on 22 / 22 steps (2026-05-04).** Headline: full Vision pipeline ran end-to-end against live Anthropic API. Sonnet 4.6 returned WRONG SUBJECT findings on the architect's test recording (correctly noted it was a CGI living room, not a motorcycle) with structured findings + suggested follow-up. Cost = $0.0354 for 5 frames. `model_used` confirmed as `claude-sonnet-4-6`. Findings expansion renders correctly in mobile, persists across cold-relaunch. All organic verifications hit during the gate: ffmpeg-missing → analysis_failed (Step 13 organic), Vision-key-missing → analysis_failed (Step 14 organic), bogus-model → analysis_failed (organic before fix-cycle-4), tier-gate 403 (Step 12 essentially closed at Step 5 organically), soft-delete 204, ProblemDetail 404 envelope.

**6 instances of the F9 failure family on Track I (3 in Phase 191B alone)**: Phase 188 HVE shape mock, Phase 190 substring-match-on-error-text, Phase 191 closure-state capture, Phase 191B serve.py deploy-path, Phase 191B Commit 1 timestamp-format (NEW failure-family entry: date-boundary latent), Phase 191B Commit 6 file:// prefix (mock-vs-real-fetch), Phase 191B Commit 2 model-string (NEW failure-family entry: tests hardcoding values that ASSERT THE BUG INTO PLACE — 14 hardcoded references across 5 files were silently masking the bug from pytest visibility). The pattern is now robust enough to merit Phase 192's lead ticket: an architectural-pattern doc + lint rule for mock-vs-runtime drift at the test-author level.

**Verified:** **151 backend Phase 191B tests pass with zero skips** at Commit 7 finalize (sample_3sec.mp4 fixture + parser fix closed the 5 previously-gated TestRealFFmpeg skips); 265 backend tests across phases 79+100+101+102+103+191B in regression sweep all green; 293 mobile Jest tests across 21 suites all green; tsc --noEmit clean every commit. Backend `pyproject.toml` 0.1.0 → 0.2.0 (Track I major-feature bump per plan v1.0.1) + `python-multipart>=0.0.20` added to `[api]` extras with inline comment explaining the FastAPI Form/UploadFile requirement. Backend project `implementation.md` 0.13.7 → 0.13.8. Schema 38 → 39. Mobile `package.json` 0.0.6 → 0.0.7. Mobile project `implementation.md` 0.0.8 → 0.0.9.

**Architect sign-off:** GATE PASSED on round 5. **Phase 192 polish backlog filed in `moto-diag-mobile/docs/FOLLOWUPS.md`**:

- **F10** — `session_repo` UTC + format consolidation (deeper bug than fix-cycle-1's video_repo timestamp fix; surfaces as Phase 178 quota tests visibly broken on calendar-month boundaries when the dev machine's local clock + UTC straddle the boundary).
- **F12** — FormData URI-prefix spec test (would have caught Commit 6 bug at jest level; mock layer needs to introspect the FormData body, not just count POST calls).
- **F13** — mobile error-mapping disambiguation: 402/quota vs 403/tier should render distinct UIs; current quota_exceeded UI is the catch-all.
- **F16 (NEW)** — runbook hygiene for API key handling (architect noted ANTHROPIC_API_KEY was leaked 4 times during this smoke cycle through pasted-in-chat values + screenshot-visible PowerShell `$env:` lines).
- **F18 (NEW)** — `image_quality_note` field not rendered in mobile findings expansion (backend has it; mobile schema mapping incomplete).
- **Issue 1 carry-forward** — post-`-wipe-data` API key recovery (consider `MOTODIAG_DEV_SEED_KEY` env var that auto-provisions on first launch with empty Keystore).
- **Phase 191 fixture loss carryover** — Phase 191's local-FS-only videos on Session #1 don't survive emulator `-wipe-data`; consider a fixture-restoration script for fresh-emulator smoke runs.
- **Doc fix** — runbook drop `--tier shop` from `motodiag apikey create` (it doesn't exist on that subcommand; tier is set separately via `motodiag subscription set --user N --tier shop`).

**Backend repo impact:** 8 commits (3 build + 4 fix-cycles + Commit 7 finalize). Schema bumped 38 → 39. **Mobile repo impact:** 3 commits (1 build + 1 fix-cycle + Commit 7 finalize). All branches deleted local; remotes were never pushed per Phase 188+ precedent.

Mobile package version 0.0.6 → 0.0.7. Mobile project `implementation.md` 0.0.8 → 0.0.9. Backend project `implementation.md` 0.13.7 → 0.13.8. Backend package `pyproject.toml` 0.1.0 → 0.2.0.

**Key finding (twofold):**

1. **Substrate-then-feature pair completion validates the split-at-plan-time pattern from Phase 191's finalize.** Phase 191 shipped the substrate; Phase 191B shipped the feature. Both passed architect-gate; total architect-gate cycles = 1 round on Phase 191 + 5 rounds on Phase 191B. The Phase 191B round count was driven by the F9 failure family (latent bugs in dependencies + the test-pinning subspecies), not by substrate-feature interaction issues — which is the right kind of cost to pay because the substrate-feature seam (the load-bearing `useSessionVideos.test.ts` assertion) held perfectly.
...
### 2026-07-17 — Mac→Windows consistency audit + doc-drift corrections

Read-only audit verifying the Mac F48 session (2026-07-01/02) left the canonical Windows system consistent. Verdict: **GREEN — not off the rails.** Findings + fixes:
- **Sync:** both repos were un-synced (each had a 25-day-old stale `.git/index.lock` from a single crashed batch git op on 2026-06-22 18:27 blocking auto-sync). Cleared stale locks; FF-pulled backend `master` → `7568c36` (F48 present), mobile `phase-196` → `1642fea` (Podfile New-Arch fix landed) + `main` → `efc8338`.
...
**Zero twist-and-go content rows shipped.** Seven content subjects died to the
documents across two adversarial refuter cycles; an eighth, already committed,
died in audit. The conclusion: the distinction lives in the transmission axis
and the filter, not in content. No successor content phase is scheduled.

Full log with dated entries:
[`docs/phases/completed/255B_phase_log.md`](docs/phases/completed/255B_phase_log.md).
Write-up: [`255B_implementation.md`](docs/phases/completed/255B_implementation.md).
